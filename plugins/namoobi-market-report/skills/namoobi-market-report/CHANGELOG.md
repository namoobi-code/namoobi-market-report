...
## v4.01.0 (plugin 1.55.0, 2026-08-31) — 💵 빅테크 조달구조(회사채·부채) 신설 · 3.1.8 하위블록 + Phase 3.8

- **핵심 지표 = 증분부채 ÷ CAPEX** (1년 새 늘어난 차입금 ÷ 같은 기간 LTM CAPEX). 실측 추이
  2024-06 −6.7%(빚 갚던 국면) → 2025-03 +8.0% → 2026-03 +42.1% → **2026-06 +44.6%**.
  자기 현금으로 짓던 데이터센터를 빚으로 짓기 시작한 전환점이 한 장에 보인다.
  (외부 참고 FactSet 2026-07-23: FY24 9% → LTM 32% — 금융리스 포함 기준이라 우리 차입금 기준과 다름)
- 서버 `fetch_bigtech_debt.py`(cron 06:45) 신설 — **SEC EDGAR XBRL companyconcept 무인증 실측**으로
  MSFT·AMZN·GOOGL·META·ORCL 의 총부채·현금성자산·회사채 발행/상환·CAPEX·영업이익·감가상각을 분기 시계열화.
  회사별 태그 상이 → 후보 중 **분기 관측 최다 태그 채택**, 10-Q YTD 누적 항목은 **누적 차분으로 분기 복원**
  (이 처리 전: GOOGL 발행 8점·META CAPEX 8점 → 후: 30점·30점). FRED IG(BAMLC0A0CM)·HY OAS 병합.
- 홈피 💵 조달구조 탭: ① 조달 요약표(합산 9분기 + 기업별 최신) ② 발행 딜 타임라인 ③ 신용 경고등
  ④ 차트 3장(총부채+비율 / 기업별 LTM 발행액 / IG·HY 가산금리).
  실측 2026-06: 오라클 순부채/EBITDA **3.69x**(유일 위험군, S&P BBB- 강등) vs 나머지 4사 순현금.
- 보고서 3.1.8 하위블록 `renderCapexFunding` + `gen_debt_charts.py`(차트 2장) + merge `m['bigtech_debt']`.
- **Phase 3.8** 신설: XBRL 에 없는 ① 발행 딜(청약배수 포함 — 배수 하락 = 수요 둔화 조기 신호)
  ② 신용등급 변경만 웹서치로 갱신 → `debt_llm.json` upsert(🧠). 추정 금지·비차단.

## v4.00.0 (plugin 1.54.0, 2026-08-31) — 📊 점유율 추이 탭 + Phase 3.7 신설

- 홈피 '점유율 추이' 탭: 릴리vs노보 원형의 '점유 대결' 11개(A급 4·B급 7)를 경쟁사 동시 라인차트+시점표+
  리더-2위 격차(확대/축소 방향)로 추적. 아마존 K뷰티·HBM 3사는 기존 서버 DB 재활용 매일 자동,
  CDMO는 캐파(만L) 시계열(삼바 36.2→84.5만L). 선별 B2 12종 점유 구도 표 동거(moat SHARES 정본).
- Phase 3.7: 보고서 실행 시 stale 배틀(신선도: 주간10·월간40·분기100일)만 웹서치 갱신 → share_llm.json
  upsert(전 플레이어 동시점 갱신 원칙·추정 금지). fetch_share.py cron 06:35.

## v3.99.0 (plugin 1.53.0, 2026-08-31) — 해자워치 '고점 질 점수' 신설 (⚪ 3등급 분화)

- 고점권(낙폭 -10% 이내) 종목을 5요소 채점(PER밴드 25·선행지표 격차 20·신고가 최근성 20·정배열 20·RSI 15, 이격+25% 감점):
  ⚪🔥 70+(추세 건강) / ⚪ 40~70 / ⚪⚠ 40미만(과열·소진 — 멀티플 의존 고점). 판정 자체가 등급이라 ⚡전환 배너에 자동 포착.
- Phase 3.6 점검 대상에 top_warn 추가(천장 신호 뉴스 점검). 첫 실측: ⚪⚠ 어드밴테스트·일루미나(37점), 최고 버텍스 67점.
- '매도 신호'가 아닌 추세 등급임을 가이드에 명시(천장은 사후 확정 — 전환이 실질 신호).
...
- Phase 3.5 뒤 신설: /api/db/moat 에서 verdict ∈ {buy, buy_m, risk} 종목(최대 6종)만 추출 →
  종목당 웹서치 1회로 해자 훼손 이슈 점검 → 유지/주의/훼손 의심 판정(추정 금지·확인 불가 시 생략) →
  data/db/moat_llm.json 저장 → Phase 5.5 sync 가 자동 업로드 → 홈피 카드 '🧠 AI 점검' 표시(moat.js).
- 비차단 — 실패 시 건너뛰며 홈피는 직전 점검 날짜 유지.
...
- **[결함] mplfinance `make_addplot` 기본값 `secondary_y="auto"`** 가 같은 패널에서 스케일이 다른 시리즈를
  임의로 우측 트윈축에 분리해 왔다(8/30 실측 3건): ① RSI 기준선 30이 우축 29~31로 분리(유령 눈금)
  ② 누적순매수 패널에서 개인만 우축 0~100(조)로 분리 — 외국인·기관과 비교 불가
  ③ 프로그램 패널에서 비차익(±5조)이 우축, 차익(±5천억)이 좌축으로 갈라져 두 선이 비슷한 진폭으로 보임(규모 10배 오독).
- **수정**: gen_kr_candle.py 의 전 addplot 에 `secondary_y=False` 명시 — 패널당 좌축 단일 스케일 고정.
  KOSPI·KOSDAQ 8/30 실데이터 재렌더로 3건 모두 해소 검증(프로그램 데이터 자체는 월별 |v| 점진 증가로 단위 오염 없음 확인).
- 참고: 8/29 아침 발송본에서 사용자가 지적한 정책금리·물가·고용·CAPEX·HBM·3.1.15 결함들은 8/29 오후
  v3.84~v3.84f 커밋으로 이미 수정 완료 — 이번 건(트윈축)만이 8/30 회차에도 남아 있던 실제 미해결분.

## v3.92.0 (plugin 1.46.0, 2026-08-29) — [부록C·D] AI 반도체에 MLCC 축 반영

- 매경ECONOMY 2026.8.26-9.1 'MLCC 어떻길래…삼성전기 방긋' 반영 — 기판(v3.90)에 이어 수동부품 축 보완.
- **[부록C] 48→49종**: 무라타(6981.T — AI 서버용 MLCC 삼성전기와 양분·가동률 90%+, 야후 7,397엔 실측) 소재/부품 그룹 추가.
  삼성전기 desc 를 MLCC 슈퍼사이클로 보강(가동률 91%·상반기 ASP +13.9%·8/1 유통가 30% 인상·모건스탠리 국내 최선호 교체).
- **[부록D] 후공정**: 무라타 카드(황색 'MLCC 양강' — 탑재량 서버 1대 2천개→VR200 NVL72 57만개) +
  범용 MLCC 후발 3인방(삼화콘덴서·아바텍·아모텍) 묶음 카드. 후발 3인방은 소형·변동성으로 종목표 편입 보류.
  PNG 재생성 — 렌더 실측 확인.

## v3.91.0 (plugin 1.45.0, 2026-08-29) — [부록G] 7대 SEED 반영 갱신 (한경BUSINESS 2026.08.26-09.01 커버스토리)

- 정책 프레임 확정 반영: '국가전략 7대 분야' → 정부 미래성장동력 **'7대 SEED'(2026.8.12 발표)** 명칭·로드맵 반영
  (SMR 경수형 2035 상용화 / 핵융합 2035 실증로→2039 100MWe / 양자 2029 오류정정 100큐비트 / 우주 2030 달 착륙선·점유율 3% /
  광물 15조원·비축 365일분 / 재생 탠덤 효율 2035년 35%).
- **SMR 계약 실체화**: 두산에너빌리티 8/14 테라파워 나트륨 핵심 기자재 계약 체결, HD현대중공업 보호·격납구조물 우선 제조사,
  삼성물산 루마니아 462MW FEED→EPC, SK이노베이션(096770)·DL이앤씨(375500) 카드 신설.
- **핵융합 상장시장 개막**: 제너럴퓨전 GFUZ 신설(2026.7 나스닥 SPAC 상장 — 첫 순수 핵융합 상장주, 야후 7.03$ 실측),
  TAE는 TMTG 60억$ 합병 추진으로 갱신, 헬리온에 뉴코어 500MW 추가, KAT(고려제강 자회사 선재)·일진파워/에이젠코어(SDS 28기) 카드 신설,
  다원시스 누적 수주 2,400억.
- 재생: 페로브스카이트 탠덤 축 신설(한화솔루션 2029 양산 목표·주성엔지니어링 33.09% 카드), LS일렉트릭 500MW HVDC·대한전선(001440) 추가.
- 광물: 고려아연 美 테네시 74억$ 통합제련소(2029 완공·연 110만t) 반영. 양자: 엑스게이트·드림시큐리티·아이씨티케이 PQC 소부대 카드.
- 바이오: 징코바이오웍스(DNA)·바이오파운드리/BCI 韓 로드맵 카드 신설. PNG 11장 재생성 — 전 슬라이스 세로비 ≤1.25 실측.

## v3.89.0 (plugin 1.44.0) — 2026-08-29 오후
- **[사고] 3.1.1 국채금리 stale 캐시 덮어쓰기 근절**: `treasury_consistent.json` 은 **생성 주체가 없는 영구 잔존 캐시**인데 asof/updated 필드조차 없어 merge 의 `LCF()` 가 매 실행 재저장하며 영원히 살아남았다. 그 안의 FMP 저품질 값(us10y 4.38·us2y 4.07)이 당일 실측(FRED DGS10 4.67·DGS2 4.20 / ^TNX 4.72)을 덮어써 3.1.1 금리표·장단기차(0.31 vs 실제 0.52)가 통째로 어긋났다(8/29 오후 재실행에서 실측 발견 — 종전 가드는 `macro.series` 가 비면 무력화돼 반쪽이었다).
  대책 3중화: ① **실측 후보 확장** — macro.series → nmr_macro.json 원본 series → fetch_us us_markets 순으로 대조값 확보 ② **신선도 검증이 불가능한 캐시는 실측이 하나라도 있으면 무조건 폐기**(임계값 방식은 2년물 괴리 0.13%p 를 놓쳐 반쪽 수정이 됐다) ③ 폐기 후 변동률·전일종가를 실측 시계열 `ret()` 로 재계산해 빈칸 방지.
- 검증: 재머지 후 us10y 4.72(1d +1.03%)·us2y 4.20(1d +0.24%)·커브 0.52, 게이트 problems 0.

## v3.90.0 (plugin 1.43.0, 2026-08-29) — [부록C·D] AI 반도체에 패키지기판(FC-BGA) 축 반영

- 매경LUXMEN 2026.09 'AI 반도체 승부처 칩에서 기판으로' 반영 — 후공정 단에 기판 축이 비어 있던 공백 보완.
- **[부록C] 46→48종**: 이비덴(4062.T — AI 서버용 FC-BGA 70~80% 과점·2028년까지 5000억엔 증설),
  삼성전기(009150 — FC-BGA+MLCC+실리콘커패시터 종합 부품·스미토모화학과 유리기판 개발) 추가.
  야후 수신 실측(이비덴 20,175엔·삼성전기 1,421,000원, 48/48 수집·errs 0).
- **[부록D] 후공정 섹션**: 부제에 시장 수치(AI 서버용 FC-BGA 92억$(2025)→185억$(2032, 360iResearch)),
  이비덴(파랑 'FC-BGA 70~80%')·삼성전기(황색 '추격 2위권')·유리기판(황색 '양산 전' — 삼성전기×스미토모·SKC 앱솔릭스
  시제품 단계, 승자 미정) 카드 3장 추가. PNG 3장 재생성 — 렌더 실측 확인.
- LG이노텍·코리아써키트·SKC 는 실적 실체(진출 추진 단계)가 약해 종목표 편입 보류 — 유리기판 카드에 이름만 언급.
- fetch_appc 출력 분모 하드코딩(39) → len(ROWS) 수정.

## v3.89.0 (plugin 1.42.0, 2026-08-29) — [부록E·F] 피지컬 AI 갱신: 유니트리 상장 편입 등 (매경LUXMEN 2026.09 반영)

- **유니트리 상장 편입**: 2026.8.19 상하이 STAR 상장(688836, 공모 150.8위안·첫날 +460%·시총 약 530억$).
  부록E 완성체 그룹에 688836.SS 추가(54→55종, 야후 수신 실측 585위안 확인), 부록F 카드 비상장→'8/19 상장' 배지 교체,
  E 서문 비상장 목록에서 제거. 시장수치 표에 IPO 1줄 + 골드만 중기전망(15억$→378억$) + 출하 집계 병기(옴디아 애지봇 39% vs 유니트리 자체 5500대+).
- **desc·카드 갱신**: 현대차(BD 소프트뱅크 지분 전량 인수 100%·2028 조지아 연 3만대·구글 딥마인드 제미나이 협력),
  레인보우로보틱스(삼성 지분 35.0% 최대주주·RB-Y1 쿠팡 실증), 테슬라(프리몬트 옵티머스 전용라인 전환·연 100만대 설계),
  LG전자(엔비디아 GR00T 기반 이족보행 휴머노이드 개발·내년 1Q 공개 목표), BYD(사업 공식화·딜러망 판매 검토),
  UBTech(워커 S1 BYD·NIO 실증), 애지봇(홍콩 IPO 절차), 어질리티(SPAC 추진) 합류, 두뇌 단에 리얼월드(RLDX-1) 회색 카드 추가.
- 부록F PNG 3장 재생성(gen_appf_physical_ai.py) — 렌더 실측 확인. 서버 fetch_appe.py 동기 배포·재수집.

## v3.88.0 (plugin 1.41.0) — 2026-08-29 (코드 주석 표기 v3.84a~f — 당시 CHANGELOG 미확인으로 번호 중복, 이 항목이 정본)
- **[사고 복구] 서버 정본 일일 누적 덮어쓰기 근절**: sync_server 가 fwd_eps(16:20 누적)·margin_debt 를 PULL 없이 PUSH 만 해, 로컬 8/13자 stale 사본이 매일 아침 서버 누적분을 덮어씀(fwd_eps 8/14~8/28 소실 실측). pull-merge(t키 union·서버값 우선) 신설 + 소실 6일치를 일자별 report_data 아카이브에서 복원·kospi_hist 백필·서버 재업로드(8/19·24~26은 영구 소실).
- **3.2.1 프로그램매매 패널 stale**: 로컬 사본(7/31) 1순위 로드 → 서버 API 1순위+write-through 로 역전(서버는 8/28·419일 정상이었음).
- **3.1.15 ①(선행이익 vs KOSPI)**: 축적 초기 판독 불가 → 누적 구간 윈도우(최소 30일~최대 2년 롤링·단기 일 눈금)로 재설계(한때 제거 후 사용자 지시로 복원).
- **3.1.1 정책금리 결정이력 차트**: 결정일 기록형 스텝 라인을 오늘(KST)까지 연장 — 최근 변경국(한국 8/27 인상)만 최신으로 보이던 오인 해소.
- **점도표**: 헤더 '6월/3월' 하드코딩 → merge 가 SEP 발표일 파싱해 latest/prev_label 동적 생성 + 차기 SEP 회의(fomc_meetings 3·6·9·12월) 자동 표기 + comment 렌더.
- **물가 차트**: 끝점 마커+제목에 최신 기준월 명시('최근 12개월' 오표기 제거). **고용 7패널**: 날짜 x축 도입 + nfp·retail 24개월 윈도우 강제(2016~ 전 이력·코로나 스파이크 혼입 수정).
- **3.1.4 OECD CLI·3.1.5 통계청 선행지수**: 공표 일정 캡션 신설(실행월-2 정지=정상 명시 — 8/29 실측 두 지표 모두 6월이 실제 최신).
- **3.1.8 CAPEX**: 표 '-' 근절(db/capex.json 풀매트릭스 2022~2027 셀 단위 선보강, 우선순위 에이전트>DB>하드코딩) + 차트 연도 순서 수정(문자열 카테고리 축 → 정수 연도 축, 2022 후치 버그).
- **3.1.9 메모리·HBM 대시보드 ②④⑥⑦⑧**: 캡션 칸 폭 72자 자동 줄바꿈(옆 칸 겹침 근절)·계약가 색 충돌/로그축/최신값 라벨·점유율 다이아 라벨 잘림·ASP 스팟 우축 분리·시장규모 헤드룸.
- **FactSet Key Metrics 6패널**: x축 한글 라벨 겹침 → 공백·괄호 기준 다줄 래핑.
...
## v3.85.0 (plugin 1.39.0, 2026-08-14) — [부록G] 7대 국가전략분야 밸류체인 관계도(해자 지도) 신설

- **[부록G] 신설**(부록F 뒤): 정부 국가전략·성장펀드 7대 분야(△SMR △핵융합 △재생에너지
  △양자 △우주·항공 △첨단바이오 △핵심광물·소부장 공급망)의 밸류체인을 분야별 3~4단(段)
  흐름 + 종목별 해자 한 줄 + 배지(파랑=독점·준독점 / 황색=과점·양강·선두 / 회색=비상장)로
  정리한 정적 이미지 11장(분야별 1~2장, docx 세로비 ≤1.3 맞춤 분할). 페이지당 1장 삽입.
- 원본: `assets/appg_strategy7_{1..11}.png` + 미리보기 `appg_strategy7.html`,
  생성기 `assets/gen_appg_strategy7.py`(구성 변경 시에만 재실행). 파일 없으면 자동 생략(비차단).
- 근거: 2026.08 웹 리서치 4갈래 병렬 실측 — 두산에너빌리티 SMR 파운드리(뉴스케일·엑스에너지·
  테라파워 주기기), 센트러스 美 HALEU 유일(DOE 9억$), CATL ESS 셀 점유 39%, MP머티리얼즈
  美 국방부 지분 15%, 알테오젠 키트루다SC 독점, 스페이스X 2026.6 상장(SPCX), 퀀티넘 상장,
  생물보안법 제정(2025.12) 등. 목차에 부록G 1줄 추가.
...
- 주 1회(월 08:00 예약) 트렌드 주간 LLM 리포트: 인스타 KR/글로벌 큐레이션 + 구글·유튜브
  주간 해설 + 네이버 쇼핑 5년 장기 해석 + 시즌·연간 리포트 발표 체크
- 산출: trends_weekly_llm.json → 홈피 Trends 탭 "주간 LLM 리포트" 섹션 렌더
- 일간 트렌드는 서버 trends_collect.py(무토큰, 매일 05:50)가 담당 — 역할 분리

## v3.82.0 (plugin 1.36.0, 2026-08-04) — 예약 catch-up 병렬 실행 중복 발송 재발방지

**증상(2026-08-04 실측)** — PC가 꺼져 06시 예약이 누락 → PC/앱 재시작(13:31) 시 Cowork 스케줄러가 누락분을 **catch-up 자동 실행**했는데, 사용자·수동 세션이 이를 모르고 13:32에 수동 재실행을 요청 → **병렬 2세션**이 같은 날짜 보고서를 각각 빌드·발송(14:14 `_1412` / 14:15 `_1413`), 수신자에게 제목이 다른 메일 2통 도착. 서버 v3.70 멱등 가드는 **첨부파일명 완전일치** 기준이라 `_HHMM` 이 다른 두 회차를 중복으로 못 잡았다.

**수정** — ① **서버 send_report_mail.py v3.71 일자 키 dedup**: `global_market_report_YYYYMMDD` 일자 키가 최근 20h 내 기발송이면 회차(_HHMM)가 달라도 SMTP 스킵·`SENT (dedup — 기발송=<파일명>)` 응답(의도적 재발송은 `--force` 로만. 서버 배포·실측 검증: 8/4 `_2359` 가상 회차 차단·8/5 통과 확인). ② **SKILL Phase 0-1 기발송·병렬 가드 신설(필수)**: 실행 초입에 서버 mail_sent.log 에서 오늘 일자 키를 grep — 기발송이면 예약 모드=즉시 종료, 일반 모드=사용자 질문(+list_sessions 로 다른 market report 세션 running 여부 보조 확인). 40여 분의 중복 수집·빌드 낭비와 초박빙 발송 레이스까지 입구에서 차단. ③ 트러블슈팅에 "예약 미실행(PC 꺼짐)=catch-up 자동 실행" 명문화.
...
**원인** — 정의가 달랐다. 스크리너(`screener_pool.py::_eps_rev`)는 **FY1+FY2 컨센 EPS 의 90일 변화율 평균**, 보고서 M7 은 **FY1 만·60일** 변화율. 기간(90 vs 60)과 대상(익년 포함 여부)이 모두 달라 GOOGL 처럼 익년 전망이 크게 개선된 종목에서 격차가 벌어졌다.
**수정(스크리너 기준으로 통일)** — ① **서버 `report_prefetch.py` 가 `eps_90d`(Yahoo epsTrend.90daysAgo)를 추가 수집**(m7_estimates, 크론 05:50·15:50) ② **merge.py 가 FY1·FY2 의 90일 변화율 평균**으로 리비전을 산출(90일값 없으면 60일 폴백·근거에 표기) ③ 스케일이 커진 만큼 **판정 임계값 ±0.2% → ±1.0%** 재조정 ④ 표의 `revision` 라벨·상세도 같은 기준으로 재작성해 "라벨은 상향인데 판정은 보합" 같은 표시 불일치 제거 ⑤ 판정 규칙 문구에 "스크리너와 동일 정의" 명시. **검증**: 신규 수집분으로 계산한 값이 스크리너와 일치(GOOGL +16.42% vs 16.4%, NVDA +11.07% vs 11.1%, AAPL +3.31% vs 3.3%).

## v3.79.0 (plugin 1.33.0, 2026-07-21) — 3.1.11 섹션 간소화 (균형안: 핵심 + 근거 요약)

**증상(사용자 지적)** — "양이 너무 많고 지저분하다". 실측 결과 이 섹션 하나가 JSON 24KB(출처 49건 10.7KB·점검카드 46행 5.8KB·`stages.note` 1,871자·`headline` 1,049자)였고, **같은 진단이 세 곳(stages.note / 핵심 한 줄 / signal_summary)에 중복**되며 **'읽는 방법' 블록이 상·하단 두 번** 나왔다.
**재설계(균형안)** — ① `stages.note` 장문 근거 **미표시**(핵심 한 줄과 중복, DB엔 보존) ② 점검 카드는 **핵심 3장 × 상위 5행**만(매일 같은 안내인 '확인 방법' 카드는 출처 각주로 갈음, 잘린 항목은 "… 외 N개" 표기) ③ `headline` **앞 3문장**까지 ④ '읽는 방법' **상단 1블록·최대 2줄**로 통합(하단 `read_monitor` 제거) ⑤ 3신호표 현재값 70자·임계선/비고 110자로 절단 ⑥ 출처 **상위 6건 + "외 N건(대시보드)"**. 단계 바·핵심 타일 4·3신호 판정표·조기경보 차트·결론 문장 등 **판단에 쓰는 요소는 전부 유지**. 데이터는 그대로 두고 렌더만 압축했으므로 대시보드·DB 정보는 손실 없음.

## v3.78.0 (plugin 1.32.0, 2026-07-21) — 3.1.9 ⑨ 패널 5일 결측 복구 (키 스키마 표류 자기치유)

**증상(사용자 지적)** — "매일 갱신" 지표인데 ⑨ HBM:DDR5 단가 격차의 현물 배율(보라 실선)이 **7/11 점 하나 → 7/17~21** 로, 7/12~16 **5일이 통째로 비었다**.
**진단** — 데이터 누락이 아니라 **키 이름 표류**였다. `db/series_mem_hbm_ddr5_gap.json` 11일치가 모두 존재하지만 스키마가 세 번 바뀌었다: 7/11 `배율(현물)`·`DDR5 현물 $/GB` → **7/12~16 `배율`(계약 기준)만** → 7/17~ `배율(계약)`·`배율(현물)` 병기. 생성기는 계약 배율만 `배율` 폴백을 갖고 있어 파란 점선은 이어졌고, **현물 배율은 폴백이 없어 5일이 결측**으로 보였다.
**수정** — ① **DB 1회 백필**: 구키 `배율`→`배율(계약)` 정규화, 결측 현물 배율은 `series_mem_dram_spot` 의 `DDR5 16Gb(2Gx8)` ÷ 2(16Gb=2GB)로 $/GB 를 구해 `HBM $/GB ÷ 값` 으로 소급 계산(검증: 7/11 47.8÷2=23.9 → 50÷23.9=2.09 로 기록값과 정확히 일치). ② **생성기 자기치유**: 앞으로 키가 또 바뀌어도 그리기 직전 dram_spot 으로 현물 배율을 복원해 차트가 비지 않게 함.

## v3.77.0 (plugin 1.31.0, 2026-07-21) — 3.1.9 ⑧ HBM 시장규모 X축 중복(2026 · 2026E) 통합

**증상(사용자 지적)** — ⑧ 패널 X축이 `2024 · 2025 · 2026 · 2026E` 로, **같은 해가 두 칸**으로 갈라져 나왔다.
**원인** — 카테고리 축을 **문자열 집합**으로 합치고 있었다: 막대(최신 전망) 시계열의 키는 `"2026E"`, 전망 빈티지(TrendForce 2024-10 / BofA 2026-01) 키는 `"2026"` 이라 `{"2026"} | {"2026E"}` 가 **서로 다른 두 카테고리**로 남았다(정렬만 연도 숫자로 하고 병합은 하지 않음).
**수정** — 카테고리 키를 **연도 정수로 정규화해 병합**하고(`_yri()`), 라벨은 전망 연도만 `2026(E)` 로 표기, 해당 막대는 **빗금(hatch)** 으로 실측/전망을 구분. 캡션에도 "빗금=전망연도" 를 명시. 데이터·값은 변경 없음(축 병합만).

## v3.76.0 (plugin 1.30.0, 2026-07-21) — 3.1.7 M7 신호 결정론화 (주관 판정이 데이터와 상반되던 문제)

**증상(사용자 지적·실측 확인)** — 같은 표 안에서 판정이 서로 모순됐다: **MSFT** 리비전 상향(+0.36%)·여력 +35.5%인데 `경계`, **META** 상향(+0.34%)·여력 +28.1%인데 `경계`, **TSLA** 당분기 EPS +18.3% 급상향인데 `위험`, 반면 **GOOGL·AMZN** 은 리비전 `혼조`인데 `긍정`. 원인은 ① SKILL 규칙이 "상향=긍정/디레이팅=경계/모멘텀 악화=위험/안정=중립" 수준의 서술이라 **임계값이 없고** ② 신호를 **에이전트 주관**에 맡겨 정성 요소(주가 부진·CAPEX 우려)가 임의로 섞였기 때문. 게다가 빌더 캡션에 **"구글·아마존·엔비디아=긍정, MS·메타=경계, 테슬라=위험, 애플=중립"이라는 고정 문장이 하드코딩**돼 표와 무관하게 매일 같은 결론이 인쇄되고 있었다.

**수정** — ① **merge.py 가 신호를 코드로 산출**: 리비전 R(연간 EPS 컨센 변화: 상향≥+0.2% / 보합 ±0.2% / 하향≤-0.2%) × 여력 U(평균목표주가 대비: 큼≥15% / 보통 5~15% / 작음<5%) 3×3 결정표 + **투자의견이 보유·매도면 1단계 하향**. 에이전트 원판정은 `signal_agent` 로 보존(감사용). ② **판정 근거를 표에 노출** — 신호 셀 아래 `리비전 상향(+0.36%) × 여력 큼(+35.5%)` 식으로 표기해 독자가 검증 가능. ③ **하드코딩 결론 문장 제거** → 실제 판정 분포를 데이터에서 집계해 렌더. ④ **게이트 req38**: 리비전 상향+여력≥15%인데 경계/위험, 또는 리비전 하향+여력<5%인데 긍정이면 **발송 차단**. ⑤ agents.md: 에이전트는 `signal` 을 만들지 말고 `rev_pct`(신설)·upside·consensus 관측치만 제공.

## v3.75.0 (plugin 1.29.0, 2026-07-21) — 3.1.10 진행 중인 달 오독 방지 (뉴스 '역대 최대' vs 차트 괴리 문의)

**증상** — 1장 뉴스는 "7월 1~20일 수출 549.3억 달러, 통관 기준 역대 최대"인데 3.1.10 차트의 2026년 07월 막대는 6월(102.2B)보다 훨씬 낮아 보여 모순처럼 읽혔다.
**진단(실측)** — **데이터·차트 모두 정상**이었다. DB `db/customs.json` 의 2026-07 은 p10=29,839,145 · p20=54,933,190 · **pm=None**(월 미종료)이고, 차트도 54,933,190 을 정확히 1~20일(연한 파랑) 계열에 그렸다. 뉴스의 '역대 최대'는 **7월 동기간(1~20일) 기준**으로, 실제 검증 결과 2026-07 54.93B 는 직전 최대 2024-07 36.88B 의 1.49배·전년 동기 36.06B 대비 +52.3%(뉴스 수치와 일치)로 **사실이 맞다**(반도체도 22.11B vs 전년 7.88B = 2.8배). 즉 **진행 중인 달(20일까지)을 완결월(31일)과 나란히 놓은 시각적 착시**가 원인.
**수정** — `gen_customs_chart.py`: ① 진행 중인 달(pm=None)의 X축 라벨을 **"2026년 07월(진행중·1~20일)"** 로 바꾸고 주황 볼드 처리 ② 해당 구간 **배경 음영**(#FEF3C7) ③ 차트 하단에 **"음영 월은 진행 중 — 완결월과 직접 비교 금지, 전년 동월 같은 구간끼리 비교"** 경고 캡션. 데이터·색 매핑·집계 로직은 변경 없음(정상이었으므로).

## v3.74.0 (plugin 1.28.0, 2026-07-21) — 렌더 사고 2건 긴급 수정 (10장 [object Object]·3.1.11 단계 바 강조 실패)
...
- **대시보드 HY 개편**: HY 섹션을 페이지 하단 → **3.1.1 금리·통화정책 내부**(국채금리 아래)로 이동, 키-값 카드 나열 → **docx 3.1.1 과 동일한 표(현재·1일·1주·1·3·6개월·1년 OAS 레벨, 현재 대비 초록/빨강)+1년 라인차트**로 교체. 값은 신규 **`db/series_hy_oas.json`**(=`nmr_hy_history.json` 사본, `fetch_kr.py` 가 매 실행 동시 갱신 → sync_server 가 db/ 업로드)에서 브라우저가 직접 계산.
- **문서**: agents.md `us_credit`={comment} 만 수집(수치 수집 불필요·HYM2EY 폐지), data-schema.md hy_spread 스키마 갱신, carry-forward 목록에서 us_credit/hy_spread 제외(항상 계산되므로 불필요).
- **DB 기준일(as_of) 자동화 (nmr_db.py)**: `set_()` 이 as_of 미지정 시 marker(날짜꼴)=데이터 최신일, 아니면 저장일을 자동 기록. `dbseries`/`dbrows` 는 **내용이 변했을 때만** 저장하도록 변경 → 기준일이 '데이터 변경 시점'을 정확히 반영(이전엔 as_of='' 로 저장돼 시계열 34종 기준일 공란). 기존 공란은 일회성 백필(로컬+서버, 시계열=마지막 데이터 날짜).
- **대시보드 DB 인벤토리**: 48종 전 항목에 **설명 열 추가**(무엇을 담는 DB인지·사용 섹션 표기), 기준일 표시 유지.
- **series_emp_gdp 오염 정리 (gen_macro_charts.py 가드)**: GDP 시계열에 날짜 포맷 4종(YYYY-MM/YYYY-MM-DD/YYYY-Qn/YYYYQn)이 혼재해 같은 분기가 중복 누적되고 YYYYQn 키에 성장률이 아닌 **GDP 레벨값**(23,082~24,180십억$) 9개가 섞여 있던 문제. dbseries 저장 전 분기 시작일(YYYY-MM-DD)로 키 정규화·레벨(|v|≥20) 배제 가드 추가 + 기존 DB 일회성 정리(로컬·서버, 56항목→20분기 성장률만). 참고: GDP 기준일이 분기 시작일(예: 2026-01-01=2026Q1)인 것은 FRED 분기 표기 관행으로 정상.
