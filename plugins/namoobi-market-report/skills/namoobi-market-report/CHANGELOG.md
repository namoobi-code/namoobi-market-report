...
- 계기: 사용자 지시(2026-09-06) — "직접수행이든 예약수행이든 주말에는 메일 발송되지 않도록". 주말은 국내 휴장이라
  데이터가 금요일치와 같고, 9/6(일) 실행은 에이전트 스톨로 11시에야 발송되는 등 주말 메일의 효용이 낮았다.
- **동작**: 주말에도 수집·빌드·게이트·서버 동기화(Phase 1~5.5)는 그대로 수행해 **대시보드는 갱신**하되, Phase 5 메일만 건너뛴다.
  (사용자 선택: "보고서는 생성·서버 업로드, 메일만 미발송".)
- **코드 게이트 2중 방어**(스킬이 깜빡해도 발송 안 됨):
  · `scripts/send_mail_server.py` v3.96 — docx 파일명 YYYYMMDD(없으면 오늘 KST)가 토·일이면 ssh 진입 전 `SKIP (weekend …)` exit 0.
    `--force` 인자로만 우회(MODE 파싱은 `--` 옵션을 건너뛰도록 변경). 서버 응답이 SKIP 이면 성공-무발송으로 종료(폴백 금지).
  · 서버 `send_report_mail.py` v3.96 — 같은 판정을 flock·dedup 앞에서 수행, `--force` 만 우회. 실측(9/6 일): 양쪽 모두 SKIP·exit 0·mail_sent.log 미기록.
- **SKILL.md**: Phase 0-0 주말 판정(`$WORK/nmr_weekend.txt`) 신설 · Phase 5 최우선 규칙(주말=Phase 5 전체 스킵·Chrome 폴백 금지·SKIP 은 실패 아님) ·
  Phase 6 보고 문구("생성 완료 — 주말 미발송(대시보드 갱신)") · 트러블슈팅 행 추가. `references/email-sending.md` 상단 배너에 동일 규칙.

## v4.04.0 (plugin 1.59.0, 2026-09-05) — AI 병목 축 3종 보강 · 부록C 55종 · 美 ETF 48종 · 병목 릴레이 배틀

- 계기: 한경MONEY 2026.08 'ETF 심층해부 — AI 메가트렌드'. 기사의 값진 부분은 종목 리스트가 아니라
  **'투자 트렌드가 병목을 따라 이동한다'(연산→메모리→광통신→전력)** 는 프레임이었다. 그 축으로 커버리지를 대조.
- **[부록C] 광통신·CPO 축 신설 (50→55종)** — 4대 하드웨어 병목의 3번째가 통째로 비어 있었다.
  '전기적' 인터커넥트(ANET·CRDO·ALAB·AVGO·MRVL)만 촘촘했고 '광학' 실물 공급자는 0종.
  실측하니 이 축이 바로 돈이 몰린 곳 — LITE +521%·COHR +195%·후지쿠라 +131%·오이솔루션 +127%(1년).
  다만 3개월은 COHR −14%·FN −22%로 조정 중이라 '병목이 이미 지나간' 신호도 같이 잡힌다.
  COHR·LITE·FN·5803.T(후지쿠라)·138080.KQ(오이솔루션).
- **생성기·빌더의 장수 하드코딩 버그 수정(조용한 누락)** — gen_appd_valuechain.py PARTS 가 6섹션·3페이지 고정이라
  광통신 섹션을 넣자 **마지막 전력·인프라가 PNG 에서 통째로 사라졌다**(FULL 도 range(6) 고정).
  빌더 renderAppendixD 도 [1,2,3] 고정이라 4번째 장을 못 읽었다. 둘 다 동적으로 교체
  (섹션 2개씩 자동 분할 · 최대 8장 탐색) + ARROWS 길이 assert. PNG 3장 → 4장.
- **[3.3.1] 美 ETF ④ AI 세부테마 15종 신설 (30→48종)** — 기사 AI ETF 맵 40종 중 보유는 11종뿐이었고
  클라우드·데이터센터·광통신·소프트웨어·보안·생성AI 는 각 0종이었다. 그룹키 `aitheme`:
  SKYY·WCLD / DTCR·TRFK / LUMA·LYTE(2026 신규상장·이력 짧음) / GRID·AIPO / CHPX / IGV / CIBR·HACK·BUG / CHAT·IGPT.
  중복분(XSD·PSI·FTXL·SOXQ·VGT·IYW·FTEC·IXN·IGM·IHAK·CLOU·BAI)은 제외. 빌더 groups ④ 신설(방어형 ④→⑤).
- **[점유율 추이] 'AI 병목 릴레이' 배틀 신설(36번째·A급 자동)** — 3.1.9 의 메모리/GPU 상대강도(2축)를
  **4축으로 일반화**. 축별 벨웨더(NVDA·MU·COHR·VRT)의 12개월 롤링 수익률을 분기 시점으로 그려
  '지금 돈이 어느 병목에 있나'를 한 화면에 보여준다. 야후 월봉 5년 자동 재계산(수집 비용 0).
  ETF 를 안 쓴 이유 = 구성이 섞이거나(SOXX 에 메모리 포함) 신규상장이라 이력이 짧음(DRAM ETF 6개월).
  **실측된 릴레이**: 2024-12 연산 +171% 선두 → 2025-06 전력 +48% → 2025-09 이후 메모리 선두,
  2026-06 +836% 정점 → 2026-09 +508%로 **꺾이는 중**(광통신도 342%→162%). 리더 둔화 = 병목 해소 국면 신호.
- 검증: fetch_appc 55/55·errs [] / fetch_us 48종·1년수익률 48/48 / fetch_share 36개 대결·bneck 16점 /
  PNG 4장 재생성 후 7.전력·인프라 12종 복구 육안 확인 / 홈피 차트 렌더 육안 확인 / verify [AppC] 하한 50→55.

## v4.03.0 (plugin 1.58.0, 2026-09-05) — [부록C·D] Bloom Energy(BE) 편입 · 온사이트 발전 축 46→50종

- 계기: 주간동아 '주식의 여왕 펠로시가 투자한 연료전지株 블룸에너지' 기사. **해자 지도 편입 여부를 먼저 실측 검토**했다.
- **판정: 해자 지도(B1/선별B2) 편입 부적합** — 기사의 근거(펠로시 투자·YTD +109%)는 해자 판단 기준이 아니고,
  실측해보니 오히려 해자의 **반대 신호**가 나왔다.
  · **매출 73%가 고객 1곳** (SEC 10-Q 2026 H1 원문 "revenue from one customer ... approximately 73%").
    전년 동기 56%(2사)보다 집중도 심화 — 협상력이 고객(하이퍼스케일러) 쪽에 있다는 뜻.
  · 'SOFC 1위'(정치형 연료전지 44%)는 **자체 발표·2차 소스 의존**. 두산퓨얼셀이 군산 연 50MW 양산 개시로 진입 중.
  · 가스터빈 대비 우위는 **리드타임(오라클 55일)뿐** — GEV·지멘스·MHI 가 2030년 30GW 로 증설 중이라
    구조적 해자가 아니라 **일시적 공급부족의 반사이익**. 블룸 자신도 인도 MTAR 핫박스 생산에 캐파 의존(양쪽 다 병목).
  · 서비스계약 10~20년이나 실제 갱신은 **연 단위** — EDA·테스터급 전환비용 근거 없음.
  · Fwd PER 47배·PSR 13.8배·YTD +138%, 공매도 10.5%, 제프리스 '2026년 이후 가시성 부족' 하향.
- **반영: 밸류체인(부록C·D)에만 수록** — '데이터센터 온사이트 전력'은 실재하는 축이고, 우리 시스템이
  GEV·CEG·VST 같은 기존 전력회사로만 보고 있어 사각지대였다. 오라클 Project Jupiter 가 가스터빈+디젤 계획을
  블룸 마이크로그리드(최대 2.85GW)로 **전환한 사건** 자체가 추적 가치가 있다.
  · `fetch_appc.py` ⑧데이터센터 전력·인프라 11→12종 (편입/불편입 판단 근거를 소스 주석으로 명시)
  · `assets/gen_appd_valuechain.py` 6.전력·인프라 섹션에 BE 카드 → PNG 3장 재생성(3번 318KB→332KB)
  · `verify_report.js` [AppC] 하한 46→50 — 그간 누락돼 있던 MLCC 후발 3인방·유리기판 확장분까지 함께 반영
  · SKILL.md 부록C 46종→50종, 그룹 내역 갱신(⑤3 ⑦10 ⑧12)
- 실측 검증: `fetch_appc.py` **50/50 수집 성공**, BE $252.87 · 1년 +360.5% · 3개월 −4.1% · 시계열 253포인트.

## v4.02.0 (plugin 1.56.0, 2026-09-04) — 해자 '유형' 5분류 + 규제 워치 · [부록J] 신설

- 계기: 뉴닉 '경제적 해자' 아티클 스터디. 개념·사례는 기존 부록/해자워치와 대부분 겹쳤으나,
  **모닝스타 5유형 분류**와 **규제 리스크** 두 축이 우리 시스템에 없다는 걸 확인해 반영.
- **왜 필요한가**: 기존 배지(B1/B2)는 해자의 '강도'만 본다. 실전에서 갈리는 건 '어떻게 무너지는가'다.
  · 원가우위 = 경쟁사 증설·보조금으로 가장 빨리 깨짐(캐파 뉴스가 곧 경보)
  · 전환비용 = 거의 안 깨짐, 단 세대 교체기에 한꺼번에 열림
  · 네트워크효과 = 오래 버티다 임계점에서 한 번에 무너짐(노키아형)
  · 무형자산 = 특허 만료일이 정해져 있어 시한이 있음 ← **노보노디스크 제외가 정확히 이 사례**
  · 효율적 규모 = 수요가 커지면 신규 진입이 열림
- 서버 `fetch_moat.py`: `MOAT_TYPE`(정의·색·취약경로) + `MOAT_TYPES`(49종 전수 태깅, 복수·앞이 주된 유형)
  + `REGULATORY`(11종 — 반독점·수출통제 등 '해자가 강해서 생기는' 리스크) 신설.
  moat.json 에 `moat_types` 사전과 행별 `mtype`·`reg` 를 실어 홈피·보고서가 같은 데이터를 쓴다.
  실측 분포: 무형자산 30 · 전환비용 18 · 원가우위 18 · 효율적규모 18 · 네트워크효과 7 · 규제 11종.
- 홈피 🏰 해자워치: 카드에 유형 배지(마우스오버 시 취약 경로) + ⚖️ 규제 리스크 경고줄 +
  유형별 필터칩(⚖️ 규제 필터 포함). moat.js?v=09040500
- 보고서: [부록I] 표에 '해자 유형' 열 추가(+ buy_z 판정 포함), **[부록J] 해자 유형 매트릭스 신설** —
  ① 유형별 정의·무너지는 방식 ② 유형 × 7대 분야 매트릭스(어느 칸에 몰려 있나) ③ ⚖️ 규제 워치 표.
  같은 태그 데이터로 자동 생성되므로 화면과 항상 일치. 목차에도 부록J 추가.
- 검증: 실제 docx 빌드 — 부록I 26행(유형 열 정상)·부록J 3개 표(유형 5행·매트릭스 5×9·규제 11행) 출력 확인.
