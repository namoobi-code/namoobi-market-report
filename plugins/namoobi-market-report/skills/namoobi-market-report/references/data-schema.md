...
- `markets.us_credit`: {comment:str} — HY 해석 코멘트만(에이전트 입력용). report_data 에는 싣지 않음: merge 가 `markets.hy_spread`={current:num, asof:str, d1..y1:num, comment:str, chart:str} 를 영구 DB(nmr_hy_history.json)에서 계산해 단일 출력. (hy_oas/hy_yield/implied_ust 폐지 — 미렌더링 중복.)
...
```json
{
  ...
  "signals": ["KOSPI200 선물 베이시스 z=+2.84 → 선물 프리미엄 확대(과매도