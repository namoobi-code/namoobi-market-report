...
- 위치: 연결폴더 `D:\claudeCowork\namoobi-market-report-server\db\<item>.json` (없으면 outputs 폴백).
...
| 3.1.3 | **고용·경기 7종(초기 실업수당 청구건수·NFP·실업률·소매판매·ISM제조·ISM서비스·GDP)+통합차트** | **[DB화]** | 각 지표 최신 발표일(청구건수=주간 목요일) | `db/employment.json` (+시계열 `db/series_emp_jobless.json`) |
| 3.1.12 | 심리(VIX·KSVKOSPI·DXY·원/달러·WTI·美10Y) | **매일 실측** | — | (지표\|의미\|시장영향 텍스트만 canonical 재사용) |
| 3.1.8 | **AI 빅테크 CAPEX(MSFT·GOOGL·AMZN·META·ORCL)+2그래프** | **[DB화]+누적 업그레이드** | 실적/가이던스 분기 | `nmr_capex.json` (기존 승계) |
| 3.1.9 | **메모리+HBM(그래프 6종·HBM 3사 EPS/PER·대시보드)** | **[DB화]+누적 업그레이드** | 분기 컨센서스/실적 | `nmr_hbm.json` (기존 승계) |
| 3.1.11 | **반도체 사이클→코스피 점검판(핵심 한 줄·조기경보 3신호표: 재고주수·계약가 QoQ·CAPEX YoY)** | **[DB화]** | 3대 신호/코스피 쏠림 변동(asof) | `db/semi_cycle.json` |
| 3.1.5 | **경기선행지수 순환변동치(표·그래프)** | **[DB화]** | 통계청 최신 발표월 | `db/leading.json` (fetch_leading 실측→DB) |
...
## 3.1.12 지표·의미·시장영향 canonical 재사용
...
- **`scripts/nmr_db.py`**: 범용 DB 모듈 — `check <item> <marker>`(reuse/due) · `get` · `set`(stdin=data) · `upsert`(누적형, keyfield 병합) · `list`. DB=`namoobi-market-report-server/db/<item>.json`={marker,as_of,data}.
