...
> **[v4.07 · 2026-09-22 재발방지] 산출 파일명·점도표 열 규약·IB 풀 폴백 (반드시 준수)**
> - **(파일명 고정)** 에이전트 산출 파일명은 merge.py 가 읽는 이름과 **글자 단위로 일치**해야 한다: News=`nmr_news.json` · **GlobalSecurities=`nmr_globalsec.json`**(`nmr_global_securities.json` 아님 — 9/22 실측 8장 전체 누락, merge 에 대체명 폴백을 넣었지만 정본은 이 이름) · USMacroExtras=`nmr_usmacro.json` · CAPEX=`nmr_capex.json` · NewsBerk=`nmr_news2.json` · KoreaSemiTheme=`nmr_semi.json` · M7=`nmr_m7.json` · SecuritiesCompose=`nmr_securities.json` · Analysis=`nmr_analysis.json` · IndexRebalance=`nmr_rebalance.json` · HBM=`nmr_hbm.json` · semi_cycle=`nmr_semi_cycle.json`. 메인세션은 발행 프롬프트에 이 파일명을 그대로 적는다.
> - **(점도표 열 의미)** `fomc_dotplot.rows[].jun` = **최신 SEP 중간값**, `mar` = **직전 SEP 중간값** — 열 이름은 역사적 잔재이고 실제 달(예: 9월/6월)은 merge 가 comment 의 날짜로 `latest_label/prev_label` 을 만들어 표기한다. **에이전트가 '3월/6월/9월' 의미로 mar/jun/sep 열을 재배치하거나 열을 추가하지 말 것**(9/22 실측: 재배치 저장 → 메인세션이 DB 값으로 수동 복원). 규칙: DB `db/dot_plot.json` rows 를 베이스로, 새 SEP(3·6·9·12월 FOMC)가 나왔을 때만 `mar←jun, jun←새 중간값` 으로 한 칸 밀고 comment 에 두 SEP 발표일(YYYY-MM-DD)을 적는다. merge 가 sep/dec 키를 감지하면 자동 정규화하지만 애초에 만들지 말 것.
> - **(IB 풀 72h 후보)** `server_ib_insights.json` 은 24h 정본 `pool` 외에 **`pool_72h`**(24h 가 빈 회사만 72h 후보)를 동봉한다(월·화 새벽 주말 공백 대응). GlobalSecurities 는 pool 이 빈 회사에 대해 pool_72h 기사를 먼저 읽고, 신선도(Daily≤D-1·Weekly≤D-3) 판정은 **기사 발행일로 스스로** 한다 — 그래도 없으면 웹서치.
> - **(증권사 DB 복구)** 서버 `broker_reports` 는 네이버 리서치가 `stock.naver.com`(SPA) 으로 이관돼 2026-09-16~22 0건이었다 → `research_watch.py v3` 가 `m.stock.naver.com/api/research/<cat>` JSON API 로 전환(스키마 불변). SecuritiesCompose 는 `server_broker_reports.json` 의 `firms[].reports`·`recent` 를 다시 1순위로 쓴다(url=`m.stock.naver.com/research/...`, pdf 포함).
...
> 2. **(req2·v3.62 개정) USMacroExtras — `fomc_meetings` 는 평면 리스트 + 최근 1년 과거 회의(실제 결정) 포함, 예정 회의는 향후 3개만**(merge 가 union·트리밍, 게이트 req16b 강제): 스키마는 `[{date,stance,note}...]` (h2_2026 같은 객체 래핑 금지 — merge 가 리스트만 인식). 과거 1년 회의는 **실제 금리 결정**(목표범위·인상/인하/동결·SEP 여부)을 note 에 기재, 미래 회의는 stance="예정". DB `db/fomc_meetings.json` 이 과거+미래 12개월 리스트를 유지한다.
> 3. **(req3) MacroAgent — 물가 YoY 시계열에 지수 레벨·null 혼입 금지**: `series.inflation` 각 라인은 YoY %만(레벨 126/148 등 혼입 금지), 값 없는 달은 **페어 자체 생략**(null 값 페어 금지 — 라인 전체 탈락 원인). gen_macro_charts 가 |v|>30·null 을 개별 필터하지만 소스가 깨끗해야 한다.
> 4. **(req4) MacroAgent — jobless(초기 실업수당) 단위=만 건**: 시계열 값은 만 건 단위(예: 21.5)로 통일(215000 같은 건 단위 금지 — 절벽 스파이크 원인). gen 이 >10000 값을 만건으로 자동 정규화 + 월간 패널 24개월·주간 52주·GDP 8분기(중복 분기 제거) 윈도우를 적용한다.
> 5. **(req5) M7OutlookAgent — `guidance` 빈값 금지**: 가이던스 미제공 기업(AAPL·GOOGL 등)은 빈칸 대신 **사실 기반 사유**("공식 수치 가이던스 미제공 관행 — 다음 실적발표에서 확인" 등)를 기재한다.
> 6. **(req6) 스파크라인 1Y 윈도우**: gen_rest_charts `spark()` 가 날짜형 시계열을 최근 365일로 잘라 그린다(3년치 GFEX 탄산리튬 등 장기 시계열이 '추세(1Y)' 라벨과 다른 색/모양이 되는 문제 수정). 시계열 소스는 기간 제한 없이 저장해도 된다.
...
> 6. **KSVKOSPI (v3.62 req8 개정)**: 1주~1년 앵커는 **merge 가 deriv_signals.db(KRX 공식 VKOSPI 일별, kr_derivatives_daily.vkospi)로 자동 계산**해 nmr_vkospi_history.json 최상위 키에 저장('-' 근절, Chrome 불필요). 당일 종가 보조 확인만 메인세션 Claude in Chrome 으로 `https://kr.investing.com/indices/kospi-volatility` 파싱. **성과 라벨은 "달"**(1일/1주/1달/3달/6달/1년) — "개월" 아님(파서 라벨 주의!). current·prev_close·anchors(1d/1w/1mo/3mo/6mo/1y)+23일 시계열 → `nmr_vkospi_history.json`. `nmr_reasons.py` 가 KSVKOSPI 행에 주입(빌더 day1pct 용 `prev_pct` 포함).
...
> **v3.39.0 — 휘발 데이터 carry-forward 계약.** HBMAgent·CapexAgent 는 **연결폴더 `namoobi-market-report-server/data/nmr_hbm.json`·`nmr_capex.json` 영구본을 베이스로 로드 → 새로 조사된 값만 갱신 → 저장**한다(merge.py `LCF()` 가 자동으로 WORK 우선·없으면 영구본·사용분 재저장). 즉 매 실행 값은 개선만 되고, 수집 실패해도 내장 예시·추정 기본값으로 되돌아가지 않는다. (손편집 JSON 은 휘발이므로 금지 — 반드시 에이전트 산출/영구본 경유.)
...
- **변동이력(빨간색, 표 위)**: nmr_changelog.py 가 비일간 지표(정책금리·물가·고용·CAPEX) 최종값을 `namoobi-market-report-server/data/nmr_valcache.json` 에 저장 → 매 실행 직전값과 비교, 변경분만 change_log 로 부착(merge 후·build 전 실행). build 가 각 표 **위에 빨간색**으로 렌더(변경 없으면 미표시).
...
- **MacroAgent series 포맷 + 출처 (v3.41 차트 완전성 — req21·26)**: gen_macro_charts 스키마(평면 숫자배열)로 산출. **반드시 인플레 5라인·고용 6패널을 모두 채운다**: `inflation`={"CPI":[..],"Core CPI":[..],"PCE":[..],"Core PCE":[..],"PPI":[..]}(각 ~24개월 YoY %), `employment`={"nfp":[..],"unemp":[..],"retail":[..],"ism_mfg":[..],"ism_svc":[..],"gdp":[..]}, + `fed_funds_5y`·`curve_10_2`+`curve_labels`.
  - **출처(검증됨)**: ① **CPI·실업률·NFP(=totalNonfarmPayroll 월차)·소매(RetailSales MoM)·GDP(realGDP 연율) = FMP `economics-indicators`**(endpoint=economics-indicators, name별, from_date≈36개월 전) — **안정적**. ② **Core CPI·PCE·Core PCE·PPI·BEI = FRED API**(CPILFESL·PCEPI·PCEPILFE·PPIFIS·T10YIE) — `fetch_macro.py`(`nmr_fred.py`, SECURITY/secrets.env `FRED_API_KEY`)가 sandbox 에서 직접 수집·YoY 계산(무키/실패 시 CSV·web_fetch 폴백). ③ **ISM 제조/서비스 = WebSearch**(월별 공개치). 없는 달은 비우고(추정 금지) DB 가 채운다.
  - **(v3.16 갱신) 샌드박스 curl 로 `api.stlouisfed.org`·`fred.stlouisfed.org` 도달 확인**(2026-07-07 실측, 단건 ~0.4s) — FRED 시계열은 `nmr_fred.py`(API 키 우선·CSV 폴백)로 **sandbox 직접 수집이 1순위**. 그래도 실패하면 web_fetch/에이전트 경로 폴백. FMP MCP 도달 가능.
  - **[Big-Arch DB 누적]** 물가·고용 series 는 `namoobi-market-report-server/db/inflation.json`·`db/employment.json` 에 **월키로 upsert 누적**(매 실행 새 달만 추가) → 한 번에 다 못 구해도 시계열이 매 실행 향상되어 항상 5라인·6패널이 그려진다. **물가 `inflation` 각 라인은 길이가 달라도 날짜축에 정렬되도록 `[["YYYY-MM",값]..]` 날짜쌍으로 산출**(gen_macro_charts 가 날짜쌍·평면 모두 처리하지만, 라인별 기간이 다르면 반드시 날짜쌍). 고용 `employment` 패널은 각자 독립이라 평면배열 허용.
  - **10Y-2Y 곡선(req2·7)**: `curve_10_2` = **FMP `treasury-rates`(endpoint=treasury-rates, from_date=1년전~당일) 의 일별 `year10`-`year2` 스프레드**(%, ~250 일별) + `curve_labels`=날짜(YYYY-MM-DD). 점 30개 초과·라벨이 날짜면 생성기가 일별 날짜축 면적차트로 렌더(GOODREPORT 수준). FRED T10Y2Y 는 `gen_curve_1y.py` 가 **FRED API 로 직접 수집(1순위, v3.16)** — FMP treasury-rates 는 폴백.
  ...
  - **기대인플레 10Y BEI 일일차트(req7)**: 일별 BEI = FRED T10YIE 를 `fetch_macro.py`(FRED API, v3.16)가 직접 수집(1순위) → 실패 시 **메인세션 Claude in Chrome 으로 `https://fred.stlouisfed.org/graph/?id=T10YIE` 또는 investing.com BEI 페이지를 `get_page_text`(JS 렌더)로 통째 읽어** 일별 시계열 파싱 → `namoobi-market-report-server/db/bei_daily.json` 에 **월키/일키 upsert 누적**(Big-Arch DB). gen 이 이 DB 로 `macro_infl_exp.png` 를 **일일선**으로 렌더. 실패 시 직전 DB 재사용.
  - **KSVKOSPI 1주~1년·스파크(req9)**: 현재값은 CNBC `.KSVKOSPI`(도달 가능)·일별캐시. 1주~1년 변동·스파크는 **메인세션 Claude in Chrome 으로 `https://kr.investing.com/indices/kospi-volatility-historical-data` 페이지를 통째(`get_page_text`)로 읽어** 일별 종가표 파싱 → `namoobi-market-report-server/data/nmr_vkospi_history.json` 에 누적 → merge `_vkospi_cache_enrich` 가 1주~1년·anchors 산출(투자 페이지가 urllib 403 이어도 Chrome 렌더는 통과).
...
- **(REQ3 · 2026-07-19 서버화3 개정) 정책금리 = 서버 daily DB — PolicyRatesAgent 폐지**: 서버 크론(market_prefetch2.py policy, 05:45·16:05)이 6개국 실측(미국 FRED DFEDTARU/L·한국 ECOS 722Y001·유로존 FRED ECBDFR·영일중 global-rates)을 `db/policy_rates.json` 에 저장하고 monthly 이력도 upsert 한다. Phase 1 은 `server_policy_rates.json` curl 캐시만 하면 **merge 가 자동 사용**(에이전트 미발행). 서버 DB 장애 시에만 구 PolicyRatesAgent 발행.
...
- **IndexRebalanceAgent → `nmr_rebalance.json`**: S&P500·나스닥100 편입/편출·일정·기준·룰변경(1차 출처 press.spglobal.com·ir.nasdaq.com). **(2026-07-19 서버화8 개정 — 발행 게이트) 서버 `server_rebalance_news.json` 의 `change_marker` 를 직전 실행 report_data 의 저장 마커(또는 db/rebalance_news 직전값)와 비교해 동일하면 이 에이전트를 발행하지 않고 DB(index_rebalance) 재사용**, 변동(신규 헤드라인)시에만 발행·조사. 비정기(M&A) 변경도 뉴스 모니터가 즉시 포착.
- **USMacroExtrasAgent → `nmr_usmacro.json`**: `bigtech_capex`(MSFT·Alphabet·Amazon·Meta 연간) — **실적연도 capex 는 FMP `statements`(`endpoint=cashflow-statement`, period=annual)의 `capitalExpenditure`(음수→절대값)로 정확 수집**(ToolSearch `+statements cashflow` 로 로드 — 현 플랜에서 statements 가용 확인됨, WebSearch 추정 대체). 차기연도 추정(**2027(E) 항상 채움**)·가이던스만 WebSearch. **[3.1.8 CAPEX 추정 출처 정책: 회사 IR·실적발표 가이던스·애널리스트 컨센서스(데이터벤더) 우선, 뉴스기사 단독 수치 사용 금지·출처 명시]** + `fomc_dotplot`(2026·2027·2028말·장기중립 각 행 **jun·mar 중간값 모두**, 빈칸 금지 — 점도표는 API 없음·WebSearch). **병합 carry-forward**: 이번 런에 `bigtech_capex`·`fomc_dotplot` 가 비면 직전 `namoobi-market-report-server/data/report_data_*.json` 에서 가져와 채움(HY 는 merge 가 영구 DB 에서 항상 계산 — carry-forward 불필요). **[P2 — v3.17] dot_plot: 매 실행 최신 FOMC SEP 발표일 마커로 `check dot_plot <SEP일>` → reuse면 점도표 `get`(조사 스킵); due면 조사·`set`. bigtech_capex 는 저렴한 마커가 없어 `nmr_cache.py gate <오늘>` 창(verify/due=실적시즌→CAPEX 조사, skip→carry-forward)으로 판정. dot_plot reuse 이고 capex skip 이면 USMacroExtras 자체를 발행하지 말 것.** **[v3.9.0] 3.2.1 빅테크 CAPEX 차트(`gen_capex_chart.py`)는 내장 기본 데이터(2023~2025 실적·2026 가이던스·2027~2029 전망E, 5개사 Microsoft·Amazon·Alphabet·Meta·Oracle, 십억 달러)로 항상 생성된다. 라이브 갱신은 `bigtech_capex` 에 `capex_series`/`rev_series`/`fcf_series`={"years":[...],"<Company>":[...]} 를 추가하면 차트가 오버라이드(없으면 내장값). 오라클 capex 는 FMP 가용 시 표·시리즈에 포함.**
...
- **NewsBerkAgent(AINews+Berkshire) → `nmr_news2.json`**: `events_calendar_longterm` ★★★ 8~10건, `berkshire` 13F(new_buys/added/reduced/exited/top_holdings≤20, 스키마 정확히), `ai_trends`(국·영 병기·**항상 정확히 10개** — 빌더 부록B 가 items[] 렌더). **(2026-07-19 서버화6) ai_trends 1순위 소스 = 서버 news_pool 의 'AI 산업' 토픽(24시간 보존, `server_news_pool.json`)** — 풀에서 선별·요약 우선, 부족분만 WebSearch. **반드시 구조**: `ai_trends={"as_of":"YYYY-MM-DD","sources_checked":[..],"items":[{"tag":"<분류>","title":"<국문 제목>","summary":"<국문 2~3문장>","title_en":"<EN title>","summary_en":"<EN 1~2 sentences>","source":"<매체>","date":"YYYY-MM-DD","url":"<링크>"}, ...**정확히 10개**]}`. AI 트렌드 items 는 **항상 10개**를 채운다(부족하면 추가 WebSearch 로 보강; 단 출처 URL 확인된 항목만 — 7~8개로 끝내지 말 것). ⚠️ **`detail` 키 금지** — 빌더는 `summary`/`title_en`/`summary_en` 를 읽으므로 국문=`summary`, 영문=`summary_en` 로 분리한다. **[P2 — v3.17] NewsBerk 는 ai_trends(상시) 때문에 매 실행 발행하되 13F 서브태스크만 게이트: 매 실행 Berkshire 최신 13F-HR 제출일(EDGAR) 마커로 `check berkshire <제출일>` → reuse면 13F는 `get` 캐시 재사용(조사 스킵), due면 조사 후 `set berkshire`. ai_trends 10개는 항상 수집.**
- **HY 스프레드**: FRED `BAMLH0A0HYM2` 일별 series(`fetch_kr.py` FRED API 직접 → equibles → 영구캐시, v3.16) → `gen_hy_chart.py` → `charts/hy_oas.png`(FRED 제공 약 3년 — ICE 라이선스 상한, API 키로도 동일). **[P2 캐시] 매 실행 FRED 최신 데이터일(월)을 마커로 `check hy_spread <YYYY-MM>` → reuse 면 히스토리 재사용; due(새 달) 면 최신 점 추가 후 `set`.**
...
- **이벤트 캘린더 (2단 수집 · 2026-07-19 서버화2 개정)**: **1순위 = 서버 `server_events_calendar.json`**(FRED 공식 발표일정 실측+FOMC DB+중앙은행 회의+만기 규칙+직전 보고서 검증분). 이 뼈대를 읽어 **검증·중요도 판정·expected_impact 작성·누락 보강만** 하고, 날짜를 웹서치로 재조사하지 않는다(뼈대에 없는 신규 이벤트만 검색).
  ...
  ② `events_calendar_longterm` — 1개월 이후 ~ 1년, **중요도 ★★★만** 6~10건, 날짜순. 일정 미확정은 "7월 말 (예정)" 식 표기.
     ⚠️ **(v3.63) `source`·`source_url` 필수** — 2.1/2.2/2.3 세 캘린더 모두 출처 열을 렌더한다. 출처 없는 이벤트는 아예 수록하지 말 것(추측 일정 금지).
...
## 5. SecuritiesAgent (한국 주요 12사 — 텔레그램 7 + Chrome 3 + 공개 웹 2 · v3.66)

**임무**: 한국 주요 증권사 12곳의 최신 리서치 시각을 `nmr_securities.json` 으로 작성. **핵심 8사는 풀(강점·채널·대표리포트·key_message·view) = kb·nh·samsung·miraeasset·korea_inv·shinhan·kiwoom·meritz, 기타 4사는 1줄 요약(key_message)** — KB·NH 는 2026 상반기 베스트 증권사 1·2위(한경비즈니스)로 핵심 승격.

**(v3.66) KB증권·NH투자증권 = 공개 웹** (텔레그램 부적합: KB=리봇 봇·시황채널 프리뷰 제한, NH=전 채널 비공개 초대링크) — `fetch_brokers_tele.py` 가 함께 수집해 `nmr_brokers_tele.json` 의 `kb`/`nh` 로 저장: KB=`rc.kbsec.com/today/index.able`(오늘의 리서치 — 모닝/마감코멘트+당일 리포트 제목 전체, 로그인 無), NH=`m.nhqv.com/research/boardList?rshPprDitCd=02`(시황·투자전략 — Global Markets Morning Brief 데일리)+`m.nhsec.com/research/newestBoardList`(최신 전체, EUC-KR). view 키: kb=`strategy_view`, nh=`global_strategy_view` (빌더 secVF·coreSet 과 동기화 — 변경 시 함께 수정).
...
**반환 JSON** (`nmr_securities.json`): firm 키 10개 + `common_themes` + `investor_type_recommendation`. 각 firm `{strength, channels:[], key_reports:[{title,(url),date}], key_message, <view>}`. view 키: kb=`strategy_view`(v3.66), nh=`global_strategy_view`(v3.66), shinhan=`asset_allocation_view`, miraeasset=`etf_emerging_view`, samsung=`derivatives_view`, korea_inv=`ib_china_view`, kiwoom=`global_etf_view`, meritz=`sector_view`, hana=`china_view`, kyobo=`bond_view`, yuanta=`daily_view`, hyundai=`industrial_view`. (요약 4사는 `key_message` 만 필수.) `key_reports` 는 문자열/객체 배열 모두 허용.
...
**도구 (2026-07-19 서버화7 개정)**: **1순위 = 서버 `server_ib_insights.json`**(IB 5사 관련 최신 보도 풀, 24시간 이내만 보유) — 풀에 항목이 있으면 그 기사들을 web_fetch 로 읽어 하우스뷰를 구성한다. 풀이 비면(주말 등) WebSearch 주력 (예: "UBS CIO daily view", "Goldman Sachs S&P 500 target", "Morgan Stanley Mike Wilson outlook", "BlackRock weekly commentary"), mcp__workspace__web_fetch 로 공개 Insights 페이지 보강. UsStockInfo MCP `get_recommendations` 로 월가 컨센서스 보조 가능. **FMP `analyst`(`price-target-consensus`·`grades-summary`)로 미국 대형주 월가 목표주가·등급 컨센서스를 정량 수집**해 `wall_street_consensus` 보강(무료·미국 한정). (⚠️ Bigdata.com MCP 는 구독 종료로 **사용 불가** — 호출 금지.)
...
- **(v3.71 · 2026-07-21 게이트 실측) 날짜 형식·1차 통과 요령**: 게이트(verify req8)는 `key_reports[].date` 가 **완전한 YYYY-MM-DD** 이고 **실행일-7일 이내**(요일 보정 월+2/토+1/일+2)일 때만 통과 — "2026"/"2026-06" 같은 부분 날짜는 무조건 stale 처리되므로 날짜를 확정 못 한 자료는 싣지 말 것. 각사 공개 발행 주기를 노려 첫 수집에서 신선본을 확보한다: **BlackRock** BII Weekly Commentary=매주 월요일(blackrock.com/corporate/insights/blackrock-investment-institute, 페이지 publicationDate 메타태그로 날짜 확정) · **J.P. Morgan** Weekly Market Recap=매주 월요일(am.jpmorgan.com — web_fetch 가 구캐시를 주면 sandbox curl 로 재확인) · **Morgan Stanley** Thoughts on the Market=거의 매 영업일(morganstanley.com/ideas/thoughts-on-the-market) · **UBS** CIO Daily=매 영업일 · **Goldman Sachs** Briefings/Exchanges=주간(그래도 7일 내 부재 시 [] 정직 표기가 옳다 — 2026-07-21 실측 GS 최신 공개분 7/10 사례). merge(v3.71)가 동일 규칙으로 stale 을 선제 제거하지만, 애초에 신선본만 수집하는 것이 원칙.
...
- `markets.korea_themes`: [{theme, direction(▲ 강세/▼ 부정/■ 양면), comment}] — 순환매 관점 주요 테마(반도체·조선·방산·전력[전력기기·송배전·ESS·원전]·증권·로봇[피지컬AI]·우주·**건설(v3.50)**·**건설기계·항공·정유(v3.50.1, 종전 수혜 트리오)**·**K푸드·K화장품(v3.63, 소비재 수출주)**·**2차전지(v3.65, KODEX 2차전지산업)**·**엔터(v3.65, TIGER 미디어컨텐츠)**). 방향은 정성 평가(누락 테마는 merge 가 수익률 기반 ▲/▼/■ 폴백 생성). `markets.korea_themes_intro`/`korea_themes_comment` 선택.
- `markets.us_credit`: {comment} — 美 하이일드(3.1.1) **해석 코멘트만** 작성. OAS 수치(current·d1~y1·asof)는 merge 가 영구 DB(`namoobi-market-report-server/data/nmr_hy_history.json`, FRED `BAMLH0A0HYM2` 일별)에서 자동 계산하므로 수집 불필요(hy_oas·hy_yield·implied_ust 필드 폐지 — 미렌더링). ICE 저작권상 **현재값·요약통계만** 표기.
...
> **(v3.96 · 2026-09-11 신설) S&P 100 도 수집 대상** — 2026-09-04 S&P DJI 보도자료의 S&P 100 변경(나이키·허니웰에어로스페이스·사이먼프로퍼티·콜게이트 편출 / 델·팔로알토·아리스타·샌디스크 편입, 9/21 발효)이 3.3.2 범위(S&P 500·나스닥 100)에 없어 통째로 누락됐다(사용자 지적). S&P 100 은 S&P 500 과 **같은 분기 보도자료**(press.spglobal.com "…Set to Join S&P 500; Others to Join S&P 100, …")에 함께 실리므로 같은 검색으로 확보된다. 산출 `index_rebalance.sp100` = sp500 과 **동형** {schedule[], events[{title,effective,add[],remove[]}], criteria[], criteria_note}. 편입 기준: S&P 500 구성종목 중 초대형·유동성 상위 100종, 개별 옵션 상장, 섹터 대표성. 서버 모니터(`market_prefetch2.py rebalance`)도 "S&P 100 index addition deletion"·"S&P100 편입 편출" 검색어를 추가해 마커에 반영된다. merge `_ir_norm`/`_rebal_canon`·carry-forward·빌더 `renderIndexRebalance`(■ S&P 100 (OEX) 블록, S&P 500 뒤)·대시보드 app.js 모두 sp100 인식.
**임무**: S&P 500·**S&P 100(v3.96)**·나스닥 100 정기 리밸런싱의 편입/편출 종목·사업내용·사유, 적용 일정, 편입 기준, 나스닥 패스트엔트리 룰 변경을 수집.
...
- `markets.korea_themes` 순서: **반도체 · AI · 전력기기 · 조선 · 방산 · 원자력 · 증권 · 로봇 · 우주 · 건설(KODEX 건설 117700) · 건설기계(KODEX 기계장비 102960) · 항공(TIGER 여행레저 228800) · 정유(KODEX 에너지화학 117460) · K푸드(HANARO Fn K-푸드 438900) · K화장품(TIGER 화장품 228790) · 2차전지(KODEX 2차전지산업 305720) · 엔터(TIGER 미디어컨텐츠 228810)**. 각 `{theme, direction(▲/▼/■), comment}`. 건설기계·항공·정유 ETF 는 섹터 프록시(전용 ETF 부재), K푸드·K화장품(v3.63)·2차전지·엔터(v3.65)는 전용 ETF.
...
- **FRED API**(`nmr_fred.py`·SECURITY/secrets.env `FRED_API_KEY`, 무키/실패 시 `fredgraph.csv` 폴백 — 3.3.3 HY OAS 와 동일 패턴): Core CPI=`CPILFESL`·PCE=`PCEPI`·Core PCE=`PCEPILFE`·PPI=`PPIFIS`·10Y 기대인플레=`T10YIE`(월별 12개+YoY/현재값).
- **ISM 제조/서비스 PMI (2026-07-19 서버화5 개정)**: **1순위 = 서버 `server_ism_pmi.json`**(PRNewswire 헤드라인 실측 파싱 — mfg/svc value·headline·url). DB에 값이 있으면 그대로 쓰고, 없을 때만 WebSearch로 ISM/PRNewswire ISM® Report On Business 헤드라인 수치 확보(예 'Manufacturing PMI at 53.3'). **'추정' 라벨 금지**(공식 발표치이므로 실측). 미발표월(휴일 지연 등)은 직전 실측월 유지·release 날짜 정확히. **한·중 정책금리만** 무료 실시간 API 부재로 WebSearch 추정('추정' 표기). **(v3.22) KSVKOSPI(코스피 변동성지수)는 추정 금지. ★중요: investing.com 페이지는 샌드박스 urllib·Claude-in-Chrome 에서 403/차단되므로 `fetch_us.py` 직접파싱이 깨진다 → MacroAgent 가 `web_fetch`(차단 안 됨)로 `https://kr.investing.com/indices/kospi-volatility`(현재/전일/1일/1년)와 `.../kospi-volatility-historical-data`(일별 종가표→스파크·1주~6개월)를 받아 `nmr_markets.json` 의 vkospi(current·prev_close·chg·1d_pct·1y_pct·anchors)에 주입한다. web_fetch 도 실패 시 `fetch_us.py` 의 CNBC `.KSVKOSPI` + 일별캐시(`nmr_vkospi_history.json`) 폴백. merge 가 'KSVKOSPI (KOSPI Volatility)' 행에 주입.**
