...
- `markets.korea_themes`: [{theme, direction(▲ 강세/▼ 부정/■ 양면), comment}] — 순환매 관점 주요 테마(반도체·조선·방산·전력[전력기기·송배전·ESS·원전]·증권·로봇[피지컬AI]·우주·**건설(v3.50)**·**건설기계·항공·정유(v3.50.1, 종전 수혜 트리오)**·**K푸드·K화장품(v3.63, 소비재 수출주)**). 방향은 정성 평가(누락 테마는 merge 가 수익률 기반 ▲/▼/■ 폴백 생성). `markets.korea_themes_intro`/`korea_themes_comment` 선택.
...
- `markets.korea_themes` 순서: **반도체 · AI · 전력기기 · 조선 · 방산 · 원자력 · 증권 · 로봇 · 우주 · 건설(KODEX 건설 117700) · 건설기계(KODEX 기계장비 102960) · 항공(TIGER 여행레저 228800) · 정유(KODEX 에너지화학 117460) · K푸드(HANARO Fn K-푸드 438900) · K화장품(TIGER 화장품 228790)**. 각 `{theme, direction(▲/▼/■), comment}`. 건설기계·항공·정유 ETF 는 섹터 프록시(전용 ETF 부재), K푸드·K화장품은 전용 ETF(v3.63).
