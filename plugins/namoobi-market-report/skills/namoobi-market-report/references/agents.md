# 서브에이전트 상세 프롬프트 및 반환 스키마 (v3.2.3)
...
> 최종 JSON 은 outputs 하위 `nmr_<에이전트이름>.json` 파일로 bash heredoc(`<<'EOF'`) 저장하고,
> 응답으로는 **저장한 파일 경로와 1줄 요약만** 반환할 것 (긴 JSON 본문을 응답으로 출력하지 말 것 — v3.2.3 속도 규칙).
