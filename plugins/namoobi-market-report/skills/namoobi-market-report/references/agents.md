...
- **이벤트 캘린더 (2단 수집)**:
  ...
  **빅테크 신기술·신제품 발표 (v3.4.2 신규 — 꼭 알아야 할 빅 이벤트만)**: 애플 아이폰 이벤트·삼성 갤럭시 언팩·엔비디아 GTC/CES 키노트·OpenAI 신모델 발표·구글 I/O·메타 커넥트·MS Build/Ignite·MWC 등.
  - 빅테크 이벤트의 expected_impact 에는 **관련 종목·섹터 영향**을 적는다 (예: "NVDA·HBM(SK하이닉스·삼성전자) AI 밸류체인 촉매").
  - 시장 영향이 큰 것만 선별 — 일반 개발자 세션·마이너 업데이트 행사는 제외. 중장기(events_calendar_longterm)에는 ★★★급(아이폰 이벤트·GTC·CES·OpenAI 플래그십 발표 등)만.
