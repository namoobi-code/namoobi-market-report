...
| 삼성증권 | https://www.samsungpop.com/sscommon/jsp/search/research/research_pop.jsp#bm | URL 끝 `#bm` 필수. 진입 시 안내 팝업 뜨면 '확인' 클릭. 단, **네이티브 "로그인 후 이용" alert 는 Chrome MCP 로 닫히지 않아 렌더러가 멈춘다** → 그 경우 목록은 `get_page_text`/screenshot 로 읽어 최신 발행일 리포트만 캡처. |
...
| 키움증권 | **공식 텔레그램 @KiwoomResearch** (firm-wide·공개) | **Chrome 불필요** — `scripts/fetch_kiwoom_tele.py`(curl ~2초)로 최근 메시지 수집 → 메인세션이 `nmr_kiwoom_tele.json` 읽어 kiwoom 엔트리 작성 |

**수집 방법 (v3.7 하이브리드)**: **키움 = 공식 텔레그램 스크립트**(`scripts/fetch_kiwoom_tele.py`, Chrome·에이전트 불필요, 다른 bash 스크립트와 병렬 tool-call). **신한·미래에셋·삼성·한투 = 공식 사이트가 JS 렌더라 메인세션 Claude in Chrome 으로** 수집하되 ① **4사 탭을 한 번에 동시 `navigate`(병렬)** 로 대기를 겹치고 ② **`get_page_text` 전체 덤프 금지 — `javascript_tool` 로 제목·발행일·요약만 타깃 추출**(메인세션 토큰 대폭 절감). 발행일이 D-1 보다 오래된 것만 있으면 최신 1~2건을 쓰되 `key_message` 에 발행일을 명시한다.
...
- **키움 주 소스 = 텔레그램**: `fetch_kiwoom_tele.py` 가 `t.me/s/KiwoomResearch`(firm-wide 공식 채널)를 curl 파싱 → `nmr_kiwoom_tele.json`(최근 12개 메시지). 메인세션이 이를 읽어 kiwoom `key_reports`(제목·발행일)·`key_message` 작성. (신한·미래에셋·삼성·한투는 firm-wide 공개 텔레그램 미확인 → 공식 사이트 유지. 미래에셋 @miraeasset_research 는 비공개.)
