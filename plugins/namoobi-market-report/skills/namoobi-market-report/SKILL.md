...
# Namoobi Market Report (plugin v1.64.0 · SKILL v4.05.0)
...
**[부록E·F] 피지컬 AI 밸류체인 (v3.72 신설)** — 부록D 뒤. 휴머노이드 부품 생태계 **상장 54종**을 6계층(①두뇌7 ②신경·감각12 ③근육·관절17 ④골격·에너지8 ⑤가상훈련장3 ⑥완성체7)으로 묶은 추세표 + 관계도. `scripts/fetch_appe.py`(sandbox·stdlib·스레드, Phase 1 bash 병렬)가 야후 일봉 2년 → `nmr_appe.json`+`nmr_appe_series.json` → merge `m['appendix_e']` → 빌더 `renderAppendixE`(데이터 없으면 자동 생략, 통화 접두 $/¥/₩/NT$/CN¥/HK$/A$). 스파크=gen_rest_charts `charts/spark_e_<심볼(.→_)>.png`. 표 앞에 시장 규모·구조 핵심 수치표(모건스탠리 5조달러·골드만삭스 국가별 생산 87.7%·옴디아 점유 39%·액추에이터 원가 30~60%·전고체/시뮬레이터 시장) 고정 삽입. **[부록F]** = `renderAppendixF` 가 repo `assets/appf_physical_ai_{1..3}.png`(생성기 `assets/gen_appf_physical_ai.py`, weasyprint→PNG) 를 charts/ 로 IEND 검증 복사 후 3장 삽입 — 파란=독점·준독점 / 황색=과점·양강 / 회색=비상장(피겨AI·애지봇·유니트리·1X·샤르파·BD·에이로봇·알코닉·엔진AI). 구성 근거=한경비즈니스 2026.08.05-11 커버스토리 '피지컬 AI 핵심 밸류체인'. 멤버십 변경 시 fetch_appe.py `ROWS` + gen_appf_physical_ai.py `SECTIONS` 동시 갱신. 서버 병행: 크론 `fetch_appe.py`(06:50·15:50) → `data/db/appe.json` → 대시보드 DB data 탭 패널·인벤토리(`appe`).
