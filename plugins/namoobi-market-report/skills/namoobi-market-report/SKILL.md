...
# Namoobi Market Report (v3.0)

> v3.0 = global-market-report v2.1 의 운영 학습을 전부 반영한 리빌드.
> - 서브에이전트 프롬프트·반환 스키마를 `references/agents.md` 에 **완전 수록** (v2.1의 "기존 v2와 동일" 참조 누락 문제 해결)
> - Gmail 발송 절차·함정 회피를 `references/email-sending.md` 로 분리 (progressive disclosure)
> - MCP 도구명을 UUID 하드코딩 대신 **ToolSearch 키워드 탐색**으로 변경 (서버 ID 변경에 견고)
> - build_report.js v3: 입력 데이터 사전 검증, null 안전 처리, 요약(Executive Summary) 섹션 추가

## 보고서 품질 기준 (반드시 충족)

생성되는 docx 는 다음 8개 항목을 모두 포함해야 한다. 하나라도 누락되면 재작업 대상.

1. **글로벌 Top News 10** — 헤드라인 + 2~4문장 요약 + 임팩트 라벨
2. **단·중·장기 추세** — 모든 자산을 1주/1개월/3개월/6개월/1년 변화율로 제시
3. **글로벌 증시 풀커버리지** — 한국(코스피·코스닥)·미국·홍콩·중국·일본·인도·베트남·유럽
4. **매크로 지표** — 달러지수(DXY), VIX, 美 10년 국채금리
5. **원자재 풀커버리지** — 에너지(WTI·천연가스) + 금속(금·은·구리) + **농산물(옥수수·대두·밀)**
6. **주요 환율** — USD/EUR/JPY/CNY/HKD vs KRW + 원화 가치 추세
7. **암호화폐** — 시장 개요 + 공포·탐욕 지수(현재/1일/1주/1개월) + 김치프리미엄(BTC/ETH/XRP/SOL)
8. **종합 분석 + 포트폴리오** — 매크로 톤·테마·리스크 + 공격형/중립형/안정형 3개 모델 + 액션 아이템
...
- 기본 수신자: `namoobi@gmail.com`, `jaewoo.seo@mobis.com` (사용자 지정 시 대체)
