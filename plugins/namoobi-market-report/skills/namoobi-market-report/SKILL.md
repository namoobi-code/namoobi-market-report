...
- **DB**: `merge.py` 가 `db/memory.json`(스냅샷) + `db/series_mem_<표>`(날짜 union 누적 6종: dram_spot/dram_contract/nand_spot/nand_contract/hbm_asp/hbm_share) 에 적재. **서버 daily cron(08:30 KST)** 도 동일 수집기를 돌려 리포트 미실행일에도 시계열이 자란다(`sync_server.py` 가 PULL→PUSH 로 양방향 병합).
