...
**7 한국 주요 증권사(12) = 텔레그램 7 + Chrome 3 + 공개 웹 2 (v3.66 KB·NH 추가)** — 신한·키움·메리츠·하나·교보·유안타·현대차는 **공식 텔레그램**, **KB증권·NH투자증권은 공개 홈페이지**(KB=`rc.kbsec.com/today/index.able` 오늘의 리서치: 모닝/마감코멘트+당일 리포트 전체 · NH=`m.nhqv.com/research/boardList?rshPprDitCd=02` 시황·투자전략(Global Markets Morning Brief 데일리)+`m.nhsec.com/research/newestBoardList`, 둘 다 로그인 無·EUC-KR) — 모두 `scripts/fetch_brokers_tele.py`(curl·bash 병렬, Chrome 불필요; kb·nh 는 텔레그램이 봇/비공개 초대링크라 웹 수집). **(2차 req10 2026-07-18) 핵심(풀) 6사 = KB·NH·삼성·미래에셋·한투·메리츠** (신한·키움·기타 요약 폐지). 7.7 = 네이버 금융리서치 모음 — 서버 `broker_reports` DB(recent 2일치·6게시판)를 merge 가 자동 주입, 빌더가 테마별 표+링크로 렌더. 6사 key_reports 링크는 merge 가 서버 DB로 자동 보강. 삼성·미래에셋·한투는 **메인세션 Claude in Chrome — `browser_batch` 로 3탭 navigate 를 묶고 `javascript_tool` 타깃추출(단계별 screenshot 금지·토큰 절감)**(공개 리서치 페이지 정상 접속됨 — 삼성=`samsungpop … research_pop.jsp#bm`(팝업'확인'), 미래에셋=`miraeasset … list.do?categoryId=1521`, 한투=`koreainvestment … Strategy.jsp?jkGubun=99`·`34`; 상세 URL `references/agents.md`. get_page_text 덤프 금지; "미확인/로그인전용" 오판 금지). 핵심 6사 풀·기타 4사 1줄 요약.
