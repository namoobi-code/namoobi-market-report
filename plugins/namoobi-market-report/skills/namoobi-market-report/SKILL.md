...
**3.3.1 빅테크 CAPEX** — MSFT·Alphabet·Amazon·Meta 연간. 실적값은 **FMP `statements` cashflow 의 `capitalExpenditure`**(절대값)로 정확 수집, 추정연도(**2027(E) 항상 채움**)만 WebSearch. 표 전체폭. **(req8) 미확인 칸은 빈값으로 두고 "확인불가"/"미공개" 문자열 금지 — 빌더가 전 기업에서 데이터가 전무한 (전망)연도 컬럼을 통째로 드롭하고 "확인불가"→"-"로 정규화한다. 2028~2029(E)는 회사 가이던스/애널리스트 컨센서스가 확인될 때만 채우고, 없으면 빈값(해당 컬럼 자동 드롭).** **(v3.62 req6) 차트는 서버 대시보드와 동일한 4분할 회사별 라인(`gen_capex_chart.py` → charts/capex_capex·capex_rev·capex_fcf·capex_ratio.png) — Microsoft·Amazon·Alphabet·Meta·Oracle 5개사(db/capex.json 소스, report_data series 오버라이드). 구 'Meta 삭제' 필터 폐지 — 표에도 Meta 포함.**
