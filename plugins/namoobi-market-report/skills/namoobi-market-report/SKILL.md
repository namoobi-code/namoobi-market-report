...
# Namoobi Market Report (plugin v1.12.0 · SKILL v3.43.0)
...
[Phase 1: 병렬 수집 — 모든 수집 에이전트를 단일 메시지로 1회 발행 (P3 통합) · 수집 에이전트 model:sonnet]
  ...
  └─ SecuritiesAgent=삼성·미래에셋·한투 3사만 메인세션 Chrome(`browser_batch` 3탭 navigate·`javascript_tool` 타깃추출·단계별 screenshot 금지) + KOSIS OECD CLI 자료갱신일 체크 1탭(3.1.9 — reuse면 스킵); 텔레그램 7사는 fetch_brokers_tele.py. 배치 발행 직후 동시 진행
        ↓
