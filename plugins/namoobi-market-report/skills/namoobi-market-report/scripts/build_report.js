...
function renderDerivPositioning(){ const m=data.markets||{};
  ...
  children.push(p("⚠ 주의 — z 부호와 '주식에 좋다/나쁘다'는 지표마다 다르다. 베이시스·순포지션은 z(+)가 우호지만, IV 스큐는 z(+)가 비우호(하방 헤지 쇄도=방어)이고, 풋콜비율·VKOSPI는 컨트라리안이라 공포 극단(z+)이 오히려 반등 신호다. 그래서 아래 표는 z 부호가 아니라 '주식에 우호/비우호' 기준으로 정리했다(각 칸에 해당 z 부호 표기).",{size:14,bold:true,color:"B45309"}));
  simpleTable([1850,3150,2200,2300],["지표","값(値)의 의미","▲ 주식에 우호(강세)","▼ 주식에 비우호(약세)"],[
    ...
    ["IV 스큐 (⚠부호 반대)","IV(25Δ풋)−IV(25Δ콜). 양수 = 풋이 비쌈 = 하방 보호 수요","z(−) 위험선호 회복(헤지 수요 완화)","z(+) 꼬리위험 경계·헤지 쇄도 = 방어·하락 경계(급락 직전 먼저 서는 선행성)"],
  ...
  children.push(p("※ 순행(추세 동조) 지표 = 베이시스·순포지션·OI / 컨트라리안(역행) 지표 = 풋콜비율·VKOSPI(극단일수록 반대 방향) / IV 스큐는 방어 강도라 z(+)가 하락 경계.",{size:13,italics:true,color:"94A3B8"}));
