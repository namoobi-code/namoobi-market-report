...
children.push(new Paragraph({alignment:AlignmentType.CENTER,spacing:{after:120},children:[new TextRun({text:"작성: Claude AI Research (Cowork) — namoobi-market-report v3.4.0",size:22,color:"64748B"})]}));
...
["1. 글로벌 Top News 10","2. 글로벌 주요 이벤트 캘린더","3. 글로벌 증시 단·중·장기 추세","4. 매크로 지표 (DXY·VIX·美10년)","5. 원자재 (에너지·금속·희토류·농산물)","6. 주요 환율 (+달러인덱스)","7. 암호화폐","8. 한국 5대 증권사","9. 글로벌 IB (UBS·GS·JPM·MS·BlackRock)","10. 종합 분석","11. 자산별 견해","12. 추천 포트폴리오","13. 액션 아이템","14. 주의 사항 및 출처"].forEach(t=>children.push(p(t,{size:22,after:40})));
...
children.push(p("VIX·DXY·美10년 국채금리를 1주~1년 변화율로 평가한다.",{italics:true,color:"64748B"}));
const usM=(data.markets&&data.markets.us_markets)||{};
const macroRows=[new TableRow({children:["지표","현재치","1주","1개월","3개월","6개월","1년","추세 평가"].map((x,i)=>cell(x,{width:tw[i],header:true,align:AlignmentType.CENTER}))})];
const ml={vix:"VIX (공포지수)",dxy:"달러지수 DXY",us10y:"美 10년 국채금리"};
let mi=0; for(const k of ["vix","dxy","us10y"]){ macroRows.push(trendRow(ml[k],usM[k],mi)); mi++; }
children.push(makeTable(tw,macroRows));

children.push(new Paragraph({children:[new PageBreak()]}));
children.push(h("5. 원자재 종합 - 에너지·금속·희토류·농산물",1));
if (data.commodities) {
  renderMarketBlock("5.1 에너지",data.commodities.energy,{wti:"WTI 원유",brent:"Brent 원유",natgas:"천연가스"});
  renderMarketBlock("5.2 금속·희토류",data.commodities.metals,{gold:"금",silver:"은",copper:"구리",platinum:"백금",rare_earth:"희토류 (REMX)"});
  renderMarketBlock("5.3 농산물",data.commodities.agriculture,{corn:"옥수수",soybean:"대두",wheat:"밀"});
  ...
  if(data.commodities.commentary){ children.push(h("5.4 원자재 종합 코멘트",2)); children.push(p(data.commodities.commentary)); }
}
children.push(new Paragraph({children:[new PageBreak()]}));
...
if (data.crypto) {
  const c=data.crypto;
  if (c.market_overview) { children.push(h("7.1 시장 개요",2)); const mo=c.market_overview; const vol=Number(mo.total_volume_24h_usd);
  ...
  if (c.fear_greed) { children.push(h("7.2 공포 & 탐욕 지수",2)); const fg=c.fear_greed;
    const fr=[["시점","지수","분류 / 변화"],["현재",String(fg.current??"-"),fg.classification||"-"],
    ...
    ].map((r,i)=>new TableRow({children:r.map((cc,j)=>cell(cc,{width:[2400,1800,5160][j],header:i===0,alt:i>0&&i%2===0,align:AlignmentType.CENTER,bold:j===0&&i>0}))}));
    children.push(makeTable([2400,1800,5160],fr)); }
  if (c.kimchi_premium && Array.isArray(c.kimchi_premium.coins) && c.kimchi_premium.coins.length) {
    children.push(h("7.3 김치 프리미엄",2)); children.push(p(`기준 환율: 1 USD = ${c.kimchi_premium.rate_usd_krw??"-"} KRW`));
    const kr=[["코인","업비트 (KRW)","바이낸스 (USD)","프리미엄","상태"],...c.kimchi_premium.coins.map(co=>[co.symbol??"-",co.upbit_krw?`₩${Number(co.upbit_krw).toLocaleString()}`:"-",co.binance_usd?`$${co.binance_usd}`:"-",(co.premium_pct!==undefined&&co.premium_pct!==null)?`${co.premium_pct}%`:"-",co.status||"-"])].map((r,i)=>new TableRow({children:r.map((cc,j)=>cell(cc,{width:[1200,2500,2400,1660,1600][j],header:i===0,alt:i>0&&i%2===0,align:AlignmentType.CENTER,bold:(j===0||j===3)&&i>0,color:(j===3&&i>0)?pctColor(c.kimchi_premium.coins[i-1].premium_pct):undefined}))}));
    children.push(makeTable([1200,2500,2400,1660,1600],kr)); }
  if (Array.isArray(c.top_gainers) && c.top_gainers.length) {
    children.push(h("7.4 24h Top Gainers / Losers",2)); const g=(c.top_gainers||[]).slice(0,5),l=(c.top_losers||[]).slice(0,5),mx=Math.max(g.length,l.length);
...
children.push(new Paragraph({children:[new PageBreak()]}));
children.push(h("8. 한국 5대 증권사 리서치",1));
...
if (data.securities) { let idx=0;
  ...
  if(Array.isArray(data.securities.common_themes)&&data.securities.common_themes.length){ children.push(h("8.6 5사 공통 핵심 주제",2)); data.securities.common_themes.forEach(t=>children.push(bullet(t))); }
  if(data.securities.investor_type_recommendation){ children.push(h("8.7 투자자 유형별 추천 조합",2)); const rec=data.securities.investor_type_recommendation;
...
if (data.global_securities) { let gi=0;
  ...
  if(Array.isArray(data.global_securities.common_themes)&&data.global_securities.common_themes.length){ children.push(h("9.6 글로벌 IB 공통 핵심 주제",2)); data.global_securities.common_themes.forEach(t=>children.push(bullet(t))); }
  if(data.global_securities.wall_street_consensus){ children.push(h("9.7 월가 컨센서스",2)); children.push(p(data.global_securities.wall_street_consensus)); }
...
children.push(p("※ 이하 10~13장은 앞 장 데이터에 근거한 AI 의견이며 투자 권유가 아닙니다.",{italics:true,color:"B45309",size:18}));
if (data.analysis) { const a=data.analysis;
  if(a.macro_view){ children.push(h("10.1 매크로 톤",2)); children.push(p(a.macro_view)); }
  if(Array.isArray(a.key_themes)&&a.key_themes.length){ children.push(h("10.2 핵심 테마",2)); const th=[["테마","방향","코멘트"]]; a.key_themes.forEach(t=>th.push([t.theme||"-",t.direction||"-",t.comment||"-"]));
  ...
  if(Array.isArray(a.key_risks)&&a.key_risks.length){ children.push(h("10.3 핵심 리스크",2)); a.key_risks.forEach(r=>children.push(bullet(r,{color:negativeColor}))); }
}
if (data.analysis && data.analysis.asset_view) {
  children.push(new Paragraph({children:[new PageBreak()]})); children.push(h("11. 자산별 단·중·장기 견해",1)); const av=data.analysis.asset_view;
...
if (data.analysis && data.analysis.portfolios) {
  children.push(new Paragraph({children:[new PageBreak()]})); children.push(h("12. 추천 포트폴리오 (공격형 · 중립형 · 안정형)",1));
  ...
  [{key:'aggressive'},{key:'balanced'},{key:'conservative'}].forEach((o,idx)=>{ const pfo=pf[o.key]; if(!pfo)return;
    children.push(h(`12.${idx+1} ${pfo.label||o.key}`,2));
...
if (data.analysis && Array.isArray(data.analysis.action_items) && data.analysis.action_items.length) {
  children.push(new Paragraph({children:[new PageBreak()]})); children.push(h("13. 액션 아이템 - 단기·중기·장기 체크리스트",1));
  data.analysis.action_items.forEach(it=>children.push(bullet(it)));
}
children.push(new Paragraph({children:[new PageBreak()]}));
children.push(h("14. 주의 사항 및 출처",1));
children.push(p("본 보고서는 Claude AI Research(Cowork)가 공개 데이터(Yahoo Finance, CoinGecko, 한국경제, 5대 증권사·글로벌 IB 공개 리서치 등)를 자동 수집·종합해 생성한 참고용 자료입니다.",{italics:true}));
children.push(p("자동 생성(AI) 특성상 오류·환각이 포함될 수 있습니다. 1~9장은 출처 링크로 검증 가능하며, 10~13장은 AI 의견입니다.",{italics:true,color:"B45309"}));
...
const doc=new Document({ styles:{ default:{document:{run:{font:"맑은 고딕",size:22}}},
  paragraphStyles:[
    {id:"Heading1",name:"Heading 1",basedOn:"Normal",next:"Normal",quickFormat:true,run:{size:36,bold:true,font:"맑은 고딕",color:"1E3A8A"},paragraph:{spacing:{before:360,after:200},outlineLevel:0}},
    {id:"Heading2",name:"Heading 2",basedOn:"Normal",next:"Normal",quickFormat:true,run:{size:28,bold:true,font:"맑은 고딕",color:"1E40AF"},paragraph:{spacing:{before:240,after:140},outlineLevel:1}},
    {id:"Heading3",name:"Heading 3",basedOn:"Normal",next:"Normal",quickFormat:true,run:{size:24,bold:true,font:"맑은 고딕",color:"334155"},paragraph:{spacing:{before:180,after:100},outlineLevel:2}}]},
  ...
  sections:[{ properties:{page:{size:{width:12240,height:15840},margin:{top:1080,right:1080,bottom:1080,left:1080}}},
    ...
    footers:{default:new Footer({children:[new Paragraph({alignment:AlignmentType.CENTER,children:[new TextRun({text:"Page ",size:18,color:"64748B"}),new TextRun({children:[PageNumber.CURRENT],size:18,color:"64748B"}),new TextRun({text:" / ",size:18,color:"64748B"}),new TextRun({children:[PageNumber.TOTAL_PAGES],size:18,color:"64748B"}),new TextRun({text:"  |  namoobi-market-report v3.4.0",size:18,color:"64748B"})]})]})},
    children }] });
...
// EOF — namoobi-market-report v3.4.0 (VM-safe compact build; 마운트 잘림 회피용 32KB 빌더)
