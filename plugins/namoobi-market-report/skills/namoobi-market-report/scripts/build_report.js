...
function renderKoreaExtras(){ const m=data.markets||{};
  ...
  { children.push(h("3.2.3 순환매 대비 테마별 현황 (대표 ETF·추세·수익률)",3));
    ...
    const retRows=(items,prefix,descFn)=>{ const rows=[retHeader()];
      ...
      return rows; };
    // 테마 (8개 고정 순서)
    const THEME_ORDER=["반도체/AI","전력기기","조선","방산","원자력","증권","로봇","우주","건설","건설기계","항공","정유","K푸드","K화장품","2차전지","엔터"];  // (v3.50) 건설 + (v3.50.1) 건설기계·항공·정유 + (v3.63) K푸드·K화장품 + (v3.65) 2차전지·엔터 추가
...
function renderDerivPositioning(){ const m=data.markets||{};
  ...
  // (2026-07-20) ⑥ 지표 해석 가이드 — 값·z 읽는 법 (홈피 3.1.13 과 동일)
  children.push(p("⑥ 지표 해석 가이드 — 값과 z 읽는 법",{bold:true,size:18,color:"1E40AF",before:120}));
  children.push(p("z-score = (당일값 − 최근 60거래일 평균) ÷ 표준편차. 단위가 다른 지표를 같은 척도로 비교하며 |z|≥1.5 주의(신호)·|z|≥2 강함. 값은 절대 수준, z는 그 지수의 평소 대비 이탈 → 방향 판단은 z로. 여러 지표 z가 동시에 같은 방향이면 신뢰도↑.",{size:14,italics:true,color:"64748B"}));
  children.push(p("✅ 부호 통일 — 모든 지표가 z(+) = 주식에 우호 / z(−) = 비우호 로 맞춰져 있다. 원래 IV 스큐만 방향이 반대였는데(원지표 = 풋−콜이라 값이 클수록 방어), 콜−풋으로 부호를 뒤집어 표시해 통일했다. 풋콜비율·VKOSPI는 컨트라리안이라 공포 극단(z+)이 곧 반등 신호이므로 그대로 z(+)=우호가 된다. 단, 선물 OI 변화만 조건부(가격 방향과 같이 봐야 함).",{size:14,bold:true,color:"1D4ED8"}));
  simpleTable([1850,3150,2200,2300],["지표","값(値)의 의미","▲ 주식에 우호(강세)","▼ 주식에 비우호(약세)"],[
    ["선물 베이시스(bp)","(선물÷현물−1)×10⁴. 정상은 소폭(+), 이론상 ≈(금리−배당)×잔존. 만기·배당락에 구조적 변동 → 수준보다 z로.","z(+) 프리미엄 확대 = 선물 매수우위·위험선호 → 통상 현물 상승 선행","z(−) 디스카운트·역전 = 헤지매도 → 하락 압력"],
    ["레버리지(美)/외국인(韓) 순","美 헤지펀드 롱−숏(계약)·韓 외국인 순매수(억원, 일별). 구조상 레버리지펀드는 순매도 성향","z(+) 매수 쏠림(추세 동조). 외국인 순매수 급증 = KOSPI200 대표 일별 선행신호","z(−) 매도·이탈(포지션 청산·위험회피)"],
    ["자산운용(美)/기관(韓) 순","美 자산운용사 롱−숏·韓 기관 순매수. 구조상 자산운용 = 순매수(현물대용 롱) 성향","z(+) 리얼머니 매수 확대","z(−) 리얼머니 축소 = 하방 경계"],
    ["선물 OI 변화 (조건부)","미결제약정 증감. 방향은 가격과 함께 봐야(가격↑+OI↑=신규매수, 가격↑+OI↓=숏커버)","가격↑ + z(+) = 신규자금 유입 → 상승 추세 강화","가격↓ + z(+) = 하락 추세 강화 / z(−) = 디레버리징(청산)"],
    ["풋콜비율(OI) (컨트라리안)","풋÷콜 미결제. 지수옵션은 상시 풋 수요로 절대수준 높음 → 반드시 z로 판단","z(+) 과도한 공포·헤지 = 바닥권 반등 선행","z(−) 안주·탐욕 = 상단 리스크"],
    ["IV 스큐 (콜−풋)","IV(25Δ콜)−IV(25Δ풋). 양수 = 콜이 비쌈 = 위험선호. 원지표는 풋−콜(값↑=방어)이라 부호를 뒤집어 다른 지표와 방향을 통일함","z(+) 위험선호 회복(하방 헤지 수요 완화)","z(−) 꼬리위험 경계·헤지 쇄도 = 방어·하락 경계(급락 직전 먼저 서는 선행성)"],
    ["딜러 감마(GEX) (변동성 축)","딜러의 감마 순노출. (+)면 변동성 억제(눌림), (−)면 변동성 증폭","z(+) 롱감마 = 변동성 억제(시장 안정)","z(−) 숏감마 = 변동성 증폭(급변 위험)"],
    ["VKOSPI(변동성지수) (컨트라리안)","코스피200 변동성지수(공포 게이지). 옵션체인 붕괴로 못 쓰는 PCR/IV 대체 韓 변동성축(美는 VIX)","z(+) 공포 극단 = 컨트라리안 반등 소지","z(−) 변동성 진정이나, 지나치면 안주 = 상단 리스크"]
  ],{left:[0,1,2,3]});
  children.push(p("※ 순행(추세 동조) 지표 = 베이시스·순포지션·OI / 컨트라리안(역행) 지표 = 풋콜비율·VKOSPI(공포 극단이 곧 반등 신호) / IV 스큐는 원지표(풋−콜)의 부호를 뒤집어 표기.",{size:13,italics:true,color:"94A3B8"}));
...
// (v3.72) [부록E] 피지컬 AI(휴머노이드) 밸류체인 — 글로벌 개별종목 55종(6계층 그룹별 추세표, v3.89 유니트리 편입). 데이터(markets.appendix_e) 없으면 자동 생략.
function renderAppendixE(){ const e=data.markets&&data.markets.appendix_e; if(!e||!e.rows||typeof e.rows!=="object")return;
  ...
  simpleTable([1750,3450,5960],["항목","수치","내용 · 출처"],[
    ...
    ["출하 점유(2025)","옴디아: 애지봇 5168대·39% / 유니트리 자체집계 5500대+","집계 기관별 기준 불일치 — 중국계 합산 세계 출하 약 87%"],
    ["유니트리 IPO","2026.8.19 상하이 STAR — 공모 150.8위안·첫날 +460%","A주 첫 휴머노이드 상장, 시총 약 530억$ — 몸값이 자본시장에 진입"],
...
// (v3.95) [부록G] 7대 국가전략분야(7대 SEED) 밸류체인 — 글로벌 개별종목 106종(7분야 그룹별 추세표, 부록C/E 동형).
// 데이터(markets.appendix_g = fetch_appg.py 산출 nmr_appg.json) 없으면 자동 생략. 구성은 [부록H] 관계도의 상장 종목 전수.
function renderAppendixG(){ const e=data.markets&&data.markets.appendix_g; if(!e||!e.rows||typeof e.rows!=="object")return;
  const groups=Array.isArray(e.groups)?e.groups:Object.keys(e.rows);
  const GL="①②③④⑤⑥⑦⑧⑨⑩";
  const CCY={USD:"$",JPY:"¥",KRW:"₩",TWD:"NT$",CNY:"CN¥",HKD:"HK$",AUD:"A$",EUR:"€",GBP:"£",DKK:"kr",CHF:"Fr"};
  if(!groups.some(function(g){return Array.isArray(e.rows[g])&&e.rows[g].length;}))return;
  children.push(new Paragraph({children:[new PageBreak()]}));
  children.push(h("[부록G] 7대 국가전략분야 밸류체인 (글로벌 개별종목)",1));
  children.push(p("정부 미래성장동력 '7대 SEED'(2026.8.12 발표) — △SMR △핵융합 △재생에너지·전력망 △핵심광물·소재·부품·장비 공급망 △양자 △우주·항공 △첨단바이오 — 의 밸류체인 상장 종목을 분야별 추세표로 정리한다. 구성은 [부록H] 관계도의 상장 종목 전수이며, 비상장(테라파워·홀텍·카이로스파워·유렌코·스페이스X·퀀티넘 등)과 순수 수요처 빅테크(AMZN·GOOGL·META·MSFT·NVDA — [부록C]에 수록)는 제외, 분야 간 중복 종목은 첫 등장 분야에만 1회 수록한다. 접두 $=미국·¥=일본·₩=한국·CN¥=중국·HK$=홍콩·A$=호주(유럽 종목은 현지통화) 종가 기준, 수익률은 일봉 종가 기준 가격수익률(배당 제외).",{italics:true,color:"64748B"}));
  let tot=0; const ys=[];
  groups.forEach(function(g,gi){ const arr=e.rows[g]; if(!Array.isArray(arr)||!arr.length)return;
    tot+=arr.length; arr.forEach(function(x){ if(x&&x["1y_pct"]!=null)ys.push(Number(x["1y_pct"])); });
    children.push(p((GL[gi]||"■")+" "+g+" ("+arr.length+"종)",{bold:true,color:"1E40AF",before:160,size:21}));
    const items=arr.map(function(x){ const sym=String(x.code||x.symbol||"-");
      return {desc:[new TextRun({text:(x.name||sym)+"  ["+sym+"]",bold:true,size:18,color:"1D4ED8"}),new TextRun({text:(x.desc?("  — "+x.desc):""),size:15,color:"64748B"})],
        m:x,current:x.current,curPrefix:(CCY[x.ccy]||"$"),trend:String(x.trend||"-"),chart:"charts/spark_g_"+sym.replace(/\./g,"_")+".png"}; });
    children.push(makeTable(TR2,trend2Rows(items))); });
  if(ys.length){ const a=ys.reduce(function(x,y){return x+y;},0)/ys.length;
    children.push(p("추세 평가: 7대 국가전략분야 밸류체인 "+tot+"종(1년 수익률 산출 "+ys.length+"종) 1년 평균 "+(a>=0?"+":"")+a.toFixed(1)+"%. 통화가 서로 달라 수익률은 현지통화 기준이며 환율 효과는 반영되지 않는다.",{bold:true,color:"0F766E",before:160})); }
  if(e.asof)children.push(p("기준: "+e.asof+" · 구성 근거: 7대 SEED(2026.8.12 발표) + [부록H] 관계도(2026.08 웹 리서치) — 멤버십 변경 시 fetch_appg.py·assets/gen_appg_strategy7.py 동시 갱신",{size:16,color:"94A3B8"}));
  children.push(p("")); }
// (v3.85→v3.95 재명명) [부록H] 7대 국가전략분야 밸류체인 관계도(해자 지도) — 정적 이미지 11장(분야별 1~2장). 원본=repo assets/appg_strategy7_{1..11}.png
// (assets/gen_appg_strategy7.py 로 구성 변경 시에만 재생성). charts/ 미존재 시 repo에서 무결성(IEND) 검증·git show 폴백 후 복사. 없으면 자동 생략(비차단).
function renderAppendixH(){ try{
  const cp=require('child_process');
  function pngOk(b){ return b&&b.length>1000&&b[0]===0x89&&b.slice(-8).toString('latin1').indexOf('IEND')>=0; }
  function loadOne(i){
  ...
  if(!imgs.some(Boolean))return;
  children.push(new Paragraph({children:[new PageBreak()]}));
  children.push(h("[부록H] 7대 국가전략분야 밸류체인 관계도 (해자 지도)",1));
  children.push(p("[부록G] 개별종목이 '왜 중요한지'를 분야별 3~4단(段) 흐름으로 잇고 종목별 해자 한 줄을 단 관계도(분야별 1~2장, 총 11장). 파란 배지=독점·준독점(대체재 사실상 없음), 황색 배지=과점·양강·선두, 회색 배지=비상장(직접 투자 불가·구조 이해용). 시세와 무관한 구조 설명용 정적 이미지(구성 변경 시 assets/gen_appg_strategy7.py 로 재생성, 2026.08 웹 리서치 기준).",{italics:true,color:"64748B"}));
  imgs.forEach(function(o,k){ if(!o)return;
    const W=o.buf.readUInt32BE(16), H=o.buf.readUInt32BE(20);
    const img=imagePara(o.rel,700,Math.round(700*H/Math.max(W,1)));
    if(img){ if(k>0)children.push(new Paragraph({children:[new PageBreak()]})); children.push(img); } });
...
function renderMoatWatch(){ try{
  ...
  const rows=(M.rows||[]).filter(r=>r.verdict==='buy'||r.verdict==='buy_z'||r.verdict==='buy_m'||r.verdict==='risk');
  if(!rows.length)return;
  const VN={buy:"🟢 일시적 빠짐 후보",buy_z:"🟢~ 빠짐 후보(선행지표 중립)",buy_m:"🟢 빠짐 후보(수동확인)",risk:"🔴 선행지표 동반 악화"};
  const MT=M.moat_types||{}; const MTN=r=>(r.mtype||[]).map(k=>(MT[k]||{}).name||k).join("·")||"-";
  ...
  simpleTable([1400,700,1500,800,750,2350,1550,2110],["종목","시장","해자 유형","52주고점比","RSI","해자 선행지표 (3개월)","판정","🧠 AI 점검"],
    rows.map(r=>{const L=LLM[r.sym];
      return [r.name,MK(r.sym),MTN(r),(r.dd!=null?r.dd.toFixed(1)+"%":"-"),(r.rsi!=null?String(r.rsi):"-"),
        (r.lead?(r.lead.name+" "+(r.lead.m3!=null?(r.lead.m3>0?"+":"")+r.lead.m3.toFixed(1)+"%":"-")):"미연결"),
        VN[r.verdict]||r.verdict,
        (L?(L.verdict_llm+" — "+(L.note||"")):"-")];}),{left:[0,2,5,7]});
...
// (v4.02 2026-09-04) [부록J] 해자 '유형' 매트릭스 — 모닝스타 5분류 × 7대 분야
//   강도(B1/B2)만 보던 해자지도에 '어떻게 무너지는가' 축을 더한다. 유형별 취약 경로가 곧 감시 대상이다.
//   데이터는 해자워치와 같은 태그(server_moat.json 의 mtype)라 화면·보고서가 항상 일치한다.
function renderMoatTypes(){ try{
  if(!fs.existsSync('server_moat.json'))return;
  const M=JSON.parse(fs.readFileSync('server_moat.json','utf-8'));
  const rows=M.rows||[], MT=M.moat_types||{};
  const keys=Object.keys(MT); if(!rows.length||!keys.length)return;
  children.push(new Paragraph({children:[new PageBreak()]}));
  children.push(h("[부록J] 해자 '유형' 매트릭스 — 어떻게 무너지는가",1));
  children.push(p("해자지도의 배지(독점·준독점)가 해자의 '강도'라면, 이 부록은 '유형'을 본다. 모닝스타 5분류(무형자산·전환비용·네트워크효과·원가우위·효율적 규모)로 "+rows.length+"종을 태깅했다. 유형을 알면 무엇을 감시해야 하는지가 정해진다 — 원가우위는 경쟁사 증설로 가장 빨리 깨지고, 전환비용은 거의 안 깨지며, 네트워크효과는 오래 버티다 임계점에서 한 번에 무너진다(노키아형). 무형자산(특허)은 만료일이 정해져 있어 시한이 있다. 노보노디스크를 유니버스에서 제외한 것도 무형자산형 해자가 대체 기술(릴리 효능 우위)에 뚫린 사례였다.",{italics:true,color:"64748B"}));
  // ① 유형별 정의·취약 경로
  simpleTable([1900,1100,9060],["유형","종목 수","무너지는 방식 (= 감시 대상)"],
    keys.map(k=>[MT[k].name,String(rows.filter(r=>(r.mtype||[]).indexOf(k)>=0).length)+"종",MT[k].risk]),{left:[0,2]});
  children.push(p(""));
  // ② 유형 × 분야 매트릭스
  const secs=[]; rows.forEach(r=>{ if(r.sec&&secs.indexOf(r.sec)<0)secs.push(r.sec); });
  const w=[1500].concat(secs.map(()=>Math.floor(10560/Math.max(1,secs.length))));
  children.push(p("■ 유형 × 분야 — 어느 칸에 몰려 있나",{bold:true,color:"1E40AF",before:120,size:21}));
  simpleTable(w,["유형"].concat(secs),
    keys.map(k=>[MT[k].name].concat(secs.map(sc=>
      rows.filter(r=>r.sec===sc&&(r.mtype||[]).indexOf(k)>=0).map(r=>r.name).join(", ")||"—"))),{left:Array.from({length:secs.length+1},(_,i)=>i)});
  children.push(p("주된 유형이 앞에 오도록 복수 태깅했으므로 한 종목이 여러 칸에 나타날 수 있다. 칸이 비어 있다는 건 그 분야에 해당 유형의 해자가 없다는 뜻 — 예컨대 네트워크효과는 소수 분야에만 존재한다.",{size:17,color:"94A3B8"}));
  // ③ 규제 워치 — 해자가 강해서 생기는 리스크
  const reg=rows.filter(r=>r.reg);
  if(reg.length){
    children.push(p("■ ⚖️ 규제 워치 — 해자가 강해서 생기는 리스크",{bold:true,color:"B91C1C",before:160,size:21}));
    children.push(p("시장지배력이 강할수록 반독점·수출통제의 표적이 된다. 해자 자체가 소송 사유가 되는 구조라, 강도가 셀수록 이 리스크는 커진다(역상관).",{italics:true,color:"64748B"}));
    simpleTable([1600,1300,9160],["종목","분야","규제 리스크"],
      reg.map(r=>[r.name,r.sec||"-",r.reg]),{left:[0,2]});
  }
  if(M.as_of)children.push(p("기준: "+M.as_of+" · 태깅 정본: 서버 fetch_moat.py MOAT_TYPES/REGULATORY(홈피 🏰 해자워치 탭과 동일 데이터)",{size:16,color:"94A3B8"}));
}catch(e){} }
renderBerkshire();
renderAITrends();
...
renderMoatTypes(); // (v4.02) [부록J] 해자 유형 매트릭스 — 캐시 없으면 자동 생략(비차단)
