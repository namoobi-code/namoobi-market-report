...
function imagePara(relOrAbs, w, hgt){ try{
  if(w>=450){ const _k=784/w; w=Math.round(w*_k); hgt=Math.round(hgt*_k); }  // 풀폭 차트 좌우 꽉 채움(콘텐츠 784px·비율 유지)
...
let tableCount=0;
const CONTENT_W=11760; // 페이지 12240 - 좌우 margin(240*2). 모든 표를 이 폭으로 꽉 채움(비율 유지)
...
const doc=new Document({ ...(embedFontData?{fonts:[{name:FONT,data:embedFontData}]}:{}),
  ...
  sections:[{ properties:{page:{size:{width:12240,height:15840},margin:{top:1080,right:240,bottom:1080,left:240}}},
