...
function renderDerivPositioning(){ const m=data.markets||{};
  ...
  children.push(p("z-score = (당일값 − 최근 60거래일 평균) ÷ 표준편차. 단위가 다른 지표를 같은 척도로 비교하며 |z|≥1.5 주의(신호)·|z|≥2 강함. 값은 절대 수준, z는 그 지수의 평소 대비 이탈 → 방향 판단은 z로. 여러 지표 z가 동시에 같은 방향으로 극단이면 신뢰도↑. 옵션 지표(풋콜·IV스큐·GEX)는 표본이 쌓인 뒤 신뢰.",{size:14,italics:true,color:"64748B"}));
  simpleTable([1900,3300,2150,2150],["지표","값(値)의 의미","z (+) 해석","z (−) 해석"],[
    ["선물 베이시스(bp)","(선물÷현물−1)×10⁴. 정상은 소폭(+), 이론상 ≈(금리−배당)×잔존. 만기·배당락에 구조적 변동 → 수준보다 z로.","프리미엄 확대 = 선물 매수우위·위험선호 → 통상 현물 상승 선행","디스카운트·역전 = 헤지매도 → 하락 압력"],
    ["레버리지(美)/외국인(韓) 순","美 헤지펀드 롱−숏(계약)·韓 외국인 순매수(억원, 일별). 구조상 레버리지펀드는 순매도 성향","매수 쏠림(추세 동조). 외국인 순매수 급증 = KOSPI200 대표 일별 선행신호","매도·이탈(포지션 청산·위험회피)"],
    ["자산운용(美)/기관(韓) 순","美 자산운용사 롱−숏·韓 기관 순매수. 구조상 자산운용 = 순매수(현물대용 롱) 성향","리얼머니 매수 확대(우호적)","리얼머니 축소 = 하방 경계"],
    ["선물 OI 변화","미결제약정 증감. 가격과 함께 봐야(가격↑+OI↑=신규매수, 가격↑+OI↓=숏커버)","신규자금 유입 = 추세 강화","디레버리징(청산)"],
    ["풋콜비율(OI)","풋÷콜 미결제. 지수옵션은 상시 풋 수요로 절대수준 높음 → 반드시 z로 판단","과도한 공포·헤지 = 바닥권 반등 선행(컨트라리안)","안주·탐욕 = 상단 리스크"],
    ["IV 스큐","IV(25Δ풋)−IV(25Δ콜). 양수 = 풋이 비쌈 = 하방 보호 수요","꼬리위험 경계·헤지 쇄도(방어). 급락 직전 먼저 서는 선행성","위험선호 회복"],
    ["딜러 감마(GEX)","딜러의 감마 순노출. (+)면 변동성 억제(눌림), (−)면 변동성 증폭","롱감마 = 변동성 억제","숏감마 = 변동성 확대"],
    ["VKOSPI(변동성지수)","코스피200 변동성지수(공포 게이지). 옵션체인 붕괴로 못 쓰는 PCR/IV 대체 韓 변동성축(美는 VIX)","공포 극단 = 컨트라리안 반등 소지","변동성 진정 = 위험선호"]
  ],{left:[0,1,2,3]});
  children.push(p("※ 컨트라리안(역행) 지표: 풋콜비율·IV스큐·VKOSPI는 극단일수록 반대 방향(반등/하락)을 시사. 베이시스·순포지션·OI는 대체로 순행(추세 동조).",{size:13,italics:true,color:"94A3B8"}));
