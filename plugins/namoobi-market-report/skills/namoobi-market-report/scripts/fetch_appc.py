#!/usr/bin/env python3
# fetch_appc.py — [부록C] AI 반도체 밸류체인 49종 (미국 31·일본 6·한국 12) — v3.51 신설, v3.52.1 ORCL·이수페타시스·AMKR 추가,
#   v3.90 이비덴·삼성전기 추가(패키지기판 FC-BGA 축 — 매경LUXMEN 2026.09 'AI 반도체 승부처 칩에서 기판으로' 반영)
#   v3.92 무라타 추가(AI 서버용 MLCC 양강 축 — 매경ECONOMY 2026.8.26-9.1 'MLCC 어떻길래')
...
ROWS=[
 ...
 ("소재/부품","6981.T","무라타 (Murata)","AI 서버용 MLCC 삼성전기와 양분 — 가동률 90%+, 서버 1대 2천개→VR200 NVL72 57만개로 탑재량 폭증"),
 ...
 ("후공정/패키징","009150.KS","삼성전기","FC-BGA 이비덴 추격 + AI 서버용 MLCC 양강(가동률 91%·ASP +13.9%·유통가 30% 인상) — 모건스탠리 국내 최선호"),
 ...
 # (2026-09-05 신설 · 한경MONEY 2026.08 'ETF 심층해부' 반영) 광통신·CPO — AI 하드웨어 4대 병목의 3번째.
 #   그동안 부록C 는 '전기적' 인터커넥트(ANET 스위칭·CRDO SerDes·ALAB PCIe/CXL·AVGO·MRVL)만 촘촘했고
 #   '광학' 쪽 실물 공급자가 통째로 없었다. GPU 연산 속도를 서버 간 전송이 못 따라가는 구간이 병목이 되면서
 #   자금이 이 축으로 이동했다는 게 실측으로 확인된다(1년 수익률: LITE +490% · COHR +188% · 후지쿠라 +164%).
 ("광통신·CPO","COHR","Coherent (코히런트)","800G/1.6T 광트랜시버 최대 공급자 — 데이터센터 간 전송의 실물 병목"),
 ("광통신·CPO","LITE","Lumentum (루멘텀)","광모듈·레이저 — 하이퍼스케일러 직납, AI 수요로 수주 급증"),
 ("광통신·CPO","FN","Fabrinet (파브리넷)","광모듈 위탁생산 1위 — 엔비디아·시스코 물량의 실제 제조처"),
 ("광통신·CPO","5803.T","후지쿠라 (Fujikura)","데이터센터 광케이블·커넥터 — AI 배선 수요의 일본 대장"),
 ("광통신·CPO","138080.KQ","오이솔루션","국내 유일 광트랜시버 양산 — 광통신 병목의 한국 연결고리"),
 ...
 # (2026-09-05 추가) 온사이트 발전 축 — 가스터빈 백로그(GEV 116GW·2031년 인도)가 못 대는 물량을
 #   리드타임 55~90일로 가져가는 대체 경로. 오라클 Project Jupiter가 가스터빈+디젤 계획을
 #   블룸 마이크로그리드(최대 2.85GW)로 전환한 것이 대표 사례.
 #   ⚠ 해자 지도(B1/선별B2) 편입은 부적합 판정 — 매출 73%가 고객 1곳(10-Q 2026 H1),
 #     두산퓨얼셀 양산 진입, 리드타임 우위는 가스터빈 증설(2030년 30GW)로 희석. 밸류체인 참고 종목으로만 수록.
 ("데이터센터 전력·인프라","BE","Bloom Energy","SOFC 온사이트 발전 — 정치형 연료전지 점유 1위, 가스터빈 대비 리드타임 우위(오라클 55일 설치)"),
]
def yfetch(sym):
