#!/usr/bin/env python3
# fetch_appe.py — [부록E] 피지컬 AI(휴머노이드) 밸류체인 — v3.72 신설, v3.89 유니트리 편입(55종)
#   구성 근거: 한경비즈니스 2026.08.05-11 커버스토리 '피지컬 AI 핵심 밸류체인'(6계층 분해) +
#              모건스탠리 2025 선정 핵심기업 · 골드만삭스/옴디아 출하 통계 +
#              매경LUXMEN 2026.09 커버스토리(유니트리 8/19 상하이 STAR 상장·현대차 BD 100%·삼성 레인보우 35% 등).
...
#   비상장(피겨AI·애지봇·1X·샤르파·보스턴다이내믹스·에이로봇·리얼월드 등)은 [부록F] 관계도에만 표기한다.
#   (유니트리는 2026.8.19 상장 — 688836.SS 로 편입. 야후 수신 실측 확인)
...
ROWS=[
 ...
 ("근육·관절(액추에이터)","066570.KS","LG전자","엔비디아 GR00T 기반 이족보행 휴머노이드 개발(내년 1분기 공개 목표) — 세탁기 DD모터 구동 노하우"),
 ...
 # ⑥ 완성체 — 휴머노이드 메이커
 ("완성체(휴머노이드)","TSLA","Tesla","옵티머스 — 2026.2Q 프리몬트 모델S·X 라인을 전용라인으로 전환(연 100만대 설계), 텍사스 장기 1000만대"),
 ("완성체(휴머노이드)","005380.KS","현대차","보스턴다이내믹스 100% 자회사화(소프트뱅크 지분 전량 인수) — 아틀라스 2028년 조지아 공장 투입·연 3만대 생산체계, 구글 딥마인드 제미나이 협력"),
 ("완성체(휴머노이드)","688836.SS","유니트리 (Unitree)","2026.8.19 상하이 STAR 상장(첫날 +460%) — 2025년 5500대+ 출하 자체집계, R1 2만9900위안 가격 파괴"),
 ("완성체(휴머노이드)","9880.HK","UBTech","워커 S1 — BYD 공장 운반·NIO 라인 조립 실증, 중국 산업용 휴머노이드 선두권"),
 ("완성체(휴머노이드)","002594.SZ","BYD","휴머노이드 사업 진출 공식화 — 자동차 딜러망 활용 판매 검토"),
 ("완성체(휴머노이드)","XPEV","XPeng (샤오펑)","광저우 공장에서 휴머노이드 '아이언(IRON)' 소량 테스트 생산"),
 ("완성체(휴머노이드)","277810.KQ","레인보우로보틱스","삼성전자 지분 35.0% 최대주주(연결 자회사) — RB-Y1 쿠팡 물류센터 실증, RX 사업추진실 연계"),
 ("완성체(휴머노이드)","454910.KS","두산로보틱스","협동로봇 국내 1위 — 산업현장 로봇 데이터 축적"),
 # (2026-09-22 신설 · money_zine 인스타 'Physical AI 10종' 카드뉴스 계기) 산업 현장 —
 #   기존 6계층은 '아직 안 팔리는' 휴머노이드 축에 집중돼 있었다(액추에이터는 부품만, 완성체는 휴머노이드만).
 #   정작 **지금 돈을 벌고 있는 피지컬AI**(공장 머신비전·산업로봇·물류 자동화)가 통째로 빠져 있었고,
 #   화낙은 해자지도에서 나브테스코·하모닉의 선행지표로 쓰면서도 밸류체인엔 없는 모순 상태였다.
 #   ⚠ 카드뉴스가 꼽은 10종 중 편입은 3종(CGNX·TER·SYM)뿐 — 나머지(UMAC·OUST·ONDS·AEVA·SERV·AMBA)는
 #     전부 만성 적자·경쟁 난립이라 제외했다(SERV 는 매출 $7.8M 에 영업손실 $189M). AMD 는 이미 두뇌 계층에 있다.
 ("산업 현장(비전·로봇·물류)","6861.T","키엔스 (Keyence)","머신비전·FA센서 1위(14.2%) — 직판·팹리스·세계최초 신제품 70%로 경쟁사 대비 단가 2~2.5배, OPM 50%대 유지"),
 ("산업 현장(비전·로봇·물류)","CGNX","Cognex (코그넥스)","머신비전 2위(11.4%) — 물류 26%·자동차 22%, OPM 30%→12.6% 붕괴 후 반등"),
 ("산업 현장(비전·로봇·물류)","6954.T","화낙 (Fanuc)","산업로봇·CNC 세계 1위 — 해자지도가 산업로봇 업황 대리로 쓰는 기준 종목"),
 ("산업 현장(비전·로봇·물류)","6506.T","야스카와 (Yaskawa)","산업로봇·AC 서보 양강 — 모션제어의 기준"),
 ("산업 현장(비전·로봇·물류)","ABBNY","ABB","산업로봇 4강 + 전력·자동화 결합(미국 ADR)"),
 ("산업 현장(비전·로봇·물류)","TER","Teradyne (테라다인)","협동로봇 UR ~15% 1위 + AMR MiR — 단 본업 테스터는 어드밴테스트에 밀리는 쪽(SoC 23% vs 66%)"),
 ("산업 현장(비전·로봇·물류)","6383.T","다이후쿠 (Daifuku)","물류자동화 세계 1위 — 반도체 클린룸 반송까지 커버"),
 ("산업 현장(비전·로봇·물류)","SYM","Symbotic (심보틱)","AI 물류창고 자동화 — ⚠ 매출 85%가 월마트 단일고객, 영업이익 미달성"),
]
def yfetch(sym):
...
json.dump(out,open(os.path.join(OUT,"nmr_appe.json"),"w"),ensure_ascii=False,indent=1)
json.dump(series,open(os.path.join(OUT,"nmr_appe_series.json"),"w"),ensure_ascii=False)
