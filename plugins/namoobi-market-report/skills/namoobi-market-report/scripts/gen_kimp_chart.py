# -*- coding: utf-8 -*-
# 6.3 김치 프리미엄 30D 차트 (req19 2026-07-18 신규)
...
def main():
    ...
    for ax,s in zip(axes.flat, SYMS):
        ...
        cur=ys[-1]
        ax.set_title(f"{s} 김프 30D   현재 {cur:+.2f}%  ·  최고 {max(ys):+.2f}%  ·  최저 {min(ys):+.2f}%",
                     fontsize=9.5, loc="left")
        # x축 눈금: 대략 6개 날짜
        step=max(1,len(arr)//6)
    ...
    fig.suptitle("김치 프리미엄 30D — 업비트 ÷ (바이낸스 × USD/KRW) - 1 · 서버 10분 수집", fontsize=10)
    fig.tight_layout(rect=[0,0,1,0.95])
    os.makedirs("charts", exist_ok=True)
    out="charts/kimp_30d.png"
