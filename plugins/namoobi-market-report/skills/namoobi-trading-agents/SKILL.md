...
> **(v3.97) 해자워치 참고**: 심사 전 `curl -s http://namoobi.duckdns.org/api/db/moat` 1회 — 심사 종목이 해자워치 유니버스에 있으면
> 판정을 참고 근거로 쓴다(🟢 빠짐 후보 = 확신도 가점 요인 / 🔴 선행지표 동반 악화 = 감점·반려 사유에 인용). 없으면 무시(비차단).
